...
"""
You are an expert Python unittest testcase developer. Your task is to generate comprehensive, maintainable test cases based on the problem statement.

<test_generation_goals>
  <goal>Generate unit/integration tests for the given feature with conditional execution based on environment, configuration, or dependencies</goal>
  <goal>Ensure tests are self-contained, maintainable, and reproducible</goal>
  <goal>Match the exact rules, mathematical formulas, algorithms, data, and workflow specified in the problem statement</goal>
  <goal>Maintain test suite integrity even when tests are not executed</goal>
</test_generation_goals>

<test_coverage_requirements>
  <requirement type="function_coverage">
    - Test all functions declared in code skeleton and function metadata
    - Do not customize function prototypes - use exact signatures from skeleton
    - Cover all code paths and branches in each function
  </requirement>
  
  <requirement type="edge_cases">
    - Generate positive edge cases (valid inputs at boundaries)
    - Generate negative edge cases (invalid inputs)
    - Generate malformed input cases (wrong types, empty values, None)
    - Generate boundary value cases (min, max, zero, one-off values)
    - Generate value change operations (increment, decrement, reset, etc.)
  </requirement>
  
  <requirement type="data_operations">
    - Test arithmetic operations: addition, subtraction, multiplication, division, modulus
    - Test bitwise operations if applicable: AND, OR, XOR, NOT, left shift, right shift
    - Test state changes: initialization, update, reset, deletion
    - Test collection operations: append, remove, search, sort, filter
  </requirement>
</test_coverage_requirements>

<conditional_execution_rules>
  <rule type="dependency_checking">
    - Check for required dependencies at test module or class level
    - If missing, use setUp/setUpClass to check and skip gracefully
    - Include descriptive logging: "Skipping test: dependency 'X' not available"
    - Example:
      ```python
      def setUp(self):
          try:
  ...
  <rule type="platform_checking">
    - Check OS or Python version for platform-specific features
    - Use sys.platform, platform.system(), or sys.version_info
    - Example:
      ```python
      import sys
  ...
  <rule type="feature_flags">
    - Use configuration or environment variables for optional features
    - Check at runtime, not with hard-coded skip markers
    - Example:
      ```python
      import os
      if not os.getenv('ENABLE_FEATURE_X'):
          self.skipTest("Feature X not enabled")
      ```
  </rule>
  
  <rule type="deterministic_data">
    - Use deterministic test data for reproducibility
    - If using random data, seed the random number generator
    - Example:
      ```python
      import random
      random.seed(42)  # Ensures same random values each run
      ```
  </rule>
</conditional_execution_rules>

<test_structure_guidelines>
  <guideline type="organization">
    - Group related tests in the same TestCase class
    - Use descriptive test method names: test_function_name_scenario
    - One assertion focus per test method when possible
  </guideline>
  
  <guideline type="setup_teardown">
    ...
    - Example:
      ```python
      def setUp(self):
          self.test_data = create_test_object()
  ...
  <guideline type="assertions">
    - assertEqual(a, b): Check equality
    - assertTrue/assertFalse: Check boolean conditions
    - assertRaises(Exception): Check exception handling
    - assertIn/assertNotIn: Check membership
    - assertIsNone/assertIsNotNone: Check None values
    - assertAlmostEqual: For floating-point comparisons
  </guideline>
  
  <guideline type="comments">
    - Add comments explaining WHY conditional execution is applied
    - Explain HOW to modify conditions for different environments
    - Document expected behavior for each test
    - Example:
      ```python
      # Skip on Windows: feature uses Unix-specific system calls
      if platform.system() == 'Windows':
          self.skipTest("Unix-only feature")
      ```
  </guideline>
</test_structure_guidelines>

<output_requirements>
  <requirement>Do not include explanations or markdown outside the code</requirement>
  <requirement>Only respond with Python code</requirement>
  <requirement>Use only standard Python library (no external dependencies)</requirement>
  <requirement>Follow unittest framework conventions</requirement>
  <requirement>Include necessary imports at the top</requirement>
  <requirement>Format code properly with consistent indentation</requirement>
  <requirement>Minimize test cases while maintaining comprehensive coverage</requirement>
  <requirement>Ensure all tests are self-contained and can run independently</requirement>
</output_requirements>
...
import sys
import platform
from main_module import main_func, helper_func
...
class TestMainFunc(unittest.TestCase):
    ...
    def test_main_func_basic(self):
        ...
        result = main_func(self.test_value)
        self.assertEqual(result, "expected_output")
    
    def test_main_func_edge_case_zero(self):
        # Test edge case: zero input
        result = main_func(0)
        self.assertEqual(result, "zero_output")
    
    def test_main_func_negative(self):
        # Test negative input
        result = main_func(-10)
        self.assertEqual(result, "negative_output")
    
    def test_main_func_exception(self):
...
STRICT REQUIREMENT: Do not include any explanations, comments, or additional text.
"""
)
...
<workflow_steps>
  ...
  <step number="2">
    ...
    <rules>
      - Use `search_in_specified_file_v2` to drill into each file
      - Use `get_functions` or `get_classes` to get detailed context
  ...
  <step number="5">
    ...
    <rules>
      ...
      - CRITICAL: You MUST modify actual source code files (not test files) to fix the issue
      - Using only test generation tools without fixing source code will result in an empty patch error
      - Ensure you apply at least one `apply_code_edit` to a non-test source file
    </rules>
  </step>
  ...
  <step number="7">
    ...
    <rules>
      ...
      - Always check all the test cases which will be impacted with your change and ensure they don't fail
      - Re-run tests only after covering all discovered occurrences to avoid partial fixes
      - Use `run_repo_tests` or `run_code` to validate your changes
    </rules>
  </step>
  ...
  <step number="8">
    ...
    <rules>
      - Analyze the test failure output carefully to understand what went wrong
      - Identify the root cause of the failure (wrong logic, missing edge case, incorrect expected output, etc.)
      - Go back to step 2-5 to fix the identified issues
      - Do NOT propose solutions again if you already have approval - directly apply fixes
      - After fixing, return to step 7 to validate again
      - Keep iterating until all tests pass or you determine the approach needs to change
      - If the same error persists after 3 attempts, consider using `start_over` with a different approach
    </rules>
  </step>
</workflow_steps>
...
<testing_rules>
  ...
  <rule>When you need a new or focused test to reproduce or protect the fix, use the dedicated test generation tool: `generate_test_function(file_path, test_function_code, position)`</rule>
  <rule>Prefer `position="auto"` which inserts after imports or before the `if __name__ == "__main__":` block when present, falling back to append</rule>
  <rule>Generated tests (new files or appended functions) are tracked and excluded from the final patch automatically, so they must not show up in the final diff</rule>
  <rule>Keep generated tests minimal and focused on the bug and its edge cases</rule>
  <rule>Note that current test functions should be passed originally and generated test function is FAIL_TO_PASS</rule>
</testing_rules>

<python_value_type_rules>
  <type name="int">
    <rules>
      - Range: -∞ to +∞ (arbitrary precision in Python 3)
      - Edge cases: 0, -1, negative numbers, very large numbers
      - Operations: +, -, *, /, //, %, **, &amp;, |, ^, ~, &lt;&lt;, &gt;&gt;
      - Type conversion: int("123"), int(3.14) → 3
      - Common issues: Division by zero, integer overflow (not in Python 3)
    </rules>
  </type>
  ...
  <type name="str">
    <rules>
      ...
      - Edge cases: empty string "", single char, very long strings, special chars, unicode
      - Common operations: concatenation (+), repetition (*), slicing [start:end], formatting
      - Escape sequences: \n (newline), \t (tab), \\ (backslash), \" (quote)
      - Methods: strip(), split(), join(), replace(), upper(), lower(), find(), format()
      - Common issues: Encoding (UTF-8), escape character handling, multiline strings
    </rules>
  </type>
  ...
  <type name="list">
    <rules>
      ...
      - Edge cases: empty list [], single element, nested lists, mixed types
      - Indexing: 0-based, negative indices count from end
      - Methods: append(), extend(), insert(), remove(), pop(), sort(), reverse()
      - Slicing: list[start:end:step]
      - Common issues: Modifying during iteration, shallow vs deep copy, index out of range
    </rules>
  </type>
  ...
  <type name="dict">
    <rules>
      ...
      - Edge cases: empty dict {}, single entry, nested dicts, None values
      - Keys: Must be hashable (str, int, tuple), not list or dict
      - Methods: get(), keys(), values(), items(), update(), pop(), setdefault()
      - Access: dict[key] raises KeyError if missing, dict.get(key, default) returns default
      - Common issues: KeyError, modifying during iteration, key type restrictions
    </rules>
  </type>
  ...
  <type name="tuple">
    <rules>
      ...
      - Edge cases: empty tuple (), single element (item,), nested tuples
      - Use cases: Multiple return values, dictionary keys, unpacking
      - Immutable: Cannot modify elements after creation
      - Unpacking: a, b = (1, 2)
      - Common issues: Forgetting comma for single element tuple
    </rules>
  </type>
  ...
  <type name="bool">
    <rules>
      ...
      - Truthy values: Non-zero numbers, non-empty strings/lists/dicts, any object
      - Falsy values: 0, 0.0, "", [], {}, (), None, False
      - Logical operators: and, or, not
      - Comparison: ==, !=, &lt;, &gt;, &lt;=, &gt;=, is, is not
      - Common issues: Using 'is' for value comparison instead of '=='
    </rules>
  </type>
  ...
  <type name="None">
    <rules>
      ...
      - Check with: if x is None: or if x is not None:
      - Default return value for functions without explicit return
      - Common issues: Comparing with == instead of 'is', forgetting to return value
    </rules>
  </type>
  ...
  <type name="float">
    <rules>
      ...
      - Edge cases: 0.0, -0.0, negative floats, very small/large numbers, inf, -inf, nan
      - Operations: +, -, *, /, //, %, **
      - Type conversion: float("3.14"), float(3) → 3.0
      - Precision: Limited precision (typically 15-17 decimal digits)
      - Special values: float('inf'), float('-inf'), float('nan')
      - Comparison: Be careful with == due to floating-point precision issues
      - Common issues: Precision errors (0.1 + 0.2 != 0.3), comparing floats with ==
  ...
  <type name="file">
    <rules>
      - File objects for reading/writing files
      - Opening modes: 'r' (read), 'w' (write), 'a' (append), 'r+' (read+write), 'b' (binary)
      - Context manager: Use 'with open(path, mode) as f:' to ensure proper closing
      - Read methods: read(), readline(), readlines(), for line in f:
      - Write methods: write(text), writelines(lines)
      - Position: seek(position), tell()
      - Encoding: Specify encoding='utf-8' for text files
      - Common issues: Forgetting to close files, encoding errors, file not found
      - Best practice: Always use context manager (with statement) for automatic cleanup
      - Path handling: Use pathlib.Path or os.path for cross-platform compatibility
    </rules>
  </type>
  ...
  <type name="set">
    <rules>
      ...
      - Edge cases: empty set set(), single element {item}, frozen sets
      - Operations: union (|), intersection (&amp;), difference (-), symmetric_difference (^)
      - Methods: add(), remove(), discard(), update(), clear()
      - Elements must be hashable (like dict keys)
      - No indexing or slicing (unordered)
      - Common issues: Empty set is set() not {}, modifying during iteration
      - Use cases: Remove duplicates, membership testing, set operations
    </rules>
  </type>
</python_value_type_rules>

<python_function_rules>
  <function_parameters>
    <rule type="positional">Required parameters without defaults, must be provided in order</rule>
    <rule type="keyword">Can have default values, can be passed by name or position</rule>
    <rule type="varargs">*args captures variable positional arguments as tuple</rule>
    <rule type="kwargs">**kwargs captures variable keyword arguments as dict</rule>
    <rule type="parameter_order">def func(positional, *args, keyword=default, **kwargs)</rule>
  </function_parameters>
  
  <function_returns>
    <rule>Explicit return: return value or return (a, b, c) for multiple values</rule>
    <rule>Implicit return: Functions without return statement return None</rule>
    <rule>Multiple returns: Can return different types based on conditions (not recommended but valid)</rule>
    <rule>Tuple unpacking: a, b = func() when func returns (x, y)</rule>
  </function_returns>
  
  <function_scope>
    <rule>Local variables: Defined inside function, not accessible outside</rule>
    <rule>Global variables: Use 'global' keyword to modify global variables inside function</rule>
    <rule>Nonlocal variables: Use 'nonlocal' keyword for nested function scopes</rule>
    <rule>LEGB rule: Local → Enclosing → Global → Built-in scope resolution</rule>
  </function_scope>
  
  <function_best_practices>
    <rule>Single responsibility: Each function should do one thing well</rule>
    <rule>Type hints: Use type annotations for better code clarity (e.g., def func(x: int) -> str:)</rule>
    <rule>Docstrings: Document function purpose, parameters, and return values</rule>
    <rule>Error handling: Use try/except for operations that might fail</rule>
    <rule>Input validation: Check parameter types and values at function entry</rule>
    <rule>Avoid side effects: Prefer pure functions that don't modify global state</rule>
  </function_best_practices>
</python_function_rules>
...
<important_constraints>
  <constraint>If you find that the error while running the run_code or run_repo_tests tool due to missing dependencies, do not try to solve it as you don't have any internet access</constraint>
</important_constraints>
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def __init__(self, available_tools: Optional[list[str]] = [], test_runner: str = "pytest", test_runner_mode: str = "FILE"):
        ...
        self.source_code_edits_count=0  # Track actual source code edits
    ...
    def save_file(self,file_path: str, content: str)->str:
        ...
        self.source_code_edits_count += 1  # Track source code edit
        logger.info(f"Source code edit #{self.source_code_edits_count} applied to {file_path} via save_file")
        return result
    ...
    def start_over(self,problem_with_old_approach:str,new_apprach_to_try:str):
        ...
        Only use this tool when you have concluded that you have changed a lot of unrelated codes and you want to start over with new approach.
        Arguments:
    ...
    def get_final_git_patch(self) -> str:
        ...
        try:
            ...
            if not to_add:
                logger.warning("No modified or untracked files found to include in patch")
                logger.warning(f"Generated test files (excluded): {self.generated_test_files}")
                logger.warning(f"Source code edits applied: {getattr(self, 'source_code_edits_count', 0)}")
            else:
                logger.info(f"Adding {len(to_add)} files to patch: {to_add}")
            ...
            if not patch_text or patch_text.strip() == "":
                logger.error("Generated patch is empty - no actual source code changes detected")
                logger.error(f"Files to add: {to_add}")
                logger.error(f"Excluded files: {exclude}")
            ...
            case 1:
                ...
                try:
                        ...
                        if not is_error:
                            ...
                            self.source_code_edits_count += 1  # Track source code edit
                            logger.info(f"Source code edit #{self.source_code_edits_count} applied to {file_path}")
    ...
    def finish(self,investigation_summary: str):
        ...
        '''
        # Check if any source code edits were made
        if self.source_code_edits_count == 0:
            logger.warning("No source code edits were made - patch will be empty!")
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                "Error: Cannot finish without making any source code changes. You must use `apply_code_edit` to modify at least one source file before calling finish."
            )
        
        qa_response={"is_patch_correct":"yes"}
        if qa_response.get("is_patch_correct","no").lower()=="yes":
            logger.info(f"Finishing workflow with {self.source_code_edits_count} source code edits applied")
            return "finish"
        else: 
...
def post_process_instruction(instruction: str) -> tuple[str, bool]:
    """
    Post-processes instruction to mark whitespaces and empty lines explicitly.
    """
    import re

    def apply_markup(text_block: str) -> tuple[str, bool]:
        """
    ...
    # Track whether any markup was applied
    any_markup_applied = False
    
    def replace_text_block(match) -> str:
        nonlocal any_markup_applied
        text_content = match.group(1)
        processed_content, markup_applied = apply_markup(text_content)        
        if markup_applied:
            any_markup_applied = True
    ...
    if any_markup_applied:
        return processed_instruction, True

    return processed_instruction, False

def generate_initial_solution(problem_statement: str, code_skeleton: str, function_metadata: dict = None) -> str:
...
def generate_test_code(problem_statement: str, code_skeleton: str, function_metadata: dict = None) -> str:
    retry = 0
    if function_metadata:
        test_generation_messages = [
            ...
            {
                "role": "user",
                "content": f"Problem Statement:\n{problem_statement}\n\nCode skeleton: \n{code_skeleton}\n\nImportant: 1. You MUST think all possible coverages in general based on problem statement. Generate test cases for all necessary functions that can make project work smoothly like start over or reset. \n2.You MUST find edge cases and boundary values and cover them in the testcases based on problem statement.\n - Generate positive edge cases.\n - Generate malformed edge cases.\n - Generate invalid input cases like empty exceeding boundary values.\n - Generate negative edge cases.\n - Generate boundary value cases. -Generate value change like increment, decrement, addition, subtraction, multiplication, division, reset, modulus, bitwise and, bitwise or, bitwise xor, bitwise not, bitwise left shift, bitwise right shift if possible.\n\nInitial Function Metadata: \n{function_metadata}\n\nGenerate the complete and correct testcases in python files.```"
            }
        ]
    else:
        test_generation_messages = [
...
def generate_function_metatdata(problem_statement: str) -> list:

    def check_response(responses: list) -> tuple[bool, str]:
        if not isinstance(responses, list):
            return False, "Responses must be a list"

        for response in responses:
            required_keys = ["name", "usage", "parameters", "return", "exceptions"]
            for key in required_keys:
                if key not in response:
                    return False, f"Required key {key} not found in response"

            if not isinstance(response["parameters"], list):
                return False, "Parameters must be a list"

            for parameter in response["parameters"]:
                if "name" not in parameter:
                    return False, "Required key name not found in parameters"
                if "example" not in parameter:
                    return False, "Required key example not found in parameters"
                if "default_value" not in parameter:
                    return False, "Required key default_value not found in parameters"
                if "type" not in parameter:
                    return False, "Required key type not found in parameters"
                if "parameter_type" not in parameter:
                    return False, "Required key parameter_type not found in parameters"
                if "required" not in parameter:
                    return False, "Required key required not found in parameters"
                if "description" not in parameter:
                    return False, "Required key description not found in parameters"
                
                # Special handling for variadic parameters
                if parameter["name"] in ["*args", "**kwargs"]:
                    if parameter["name"] == "*args" and parameter["type"] != "varargs":
                        return False, "Parameter *args must have type 'varargs'"
                    if parameter["name"] == "**kwargs" and parameter["type"] != "kwargs":
                        return False, "Parameter **kwargs must have type 'kwargs'"
                    if parameter["parameter_type"] not in ["varargs", "kwargs"]:
                        return False, f"Invalid parameter_type for variadic parameter: {parameter['parameter_type']}"
                else:
                    # Regular parameter type validation
                    if parameter["type"] not in AVAILABLE_INSTANCE_TYPES.keys():
                        return False, f"Invalid type {parameter['type']} in parameters: {list(AVAILABLE_INSTANCE_TYPES.keys())}"
                
                # Validate parameter_type values
                valid_parameter_types = ["positional", "keyword", "varargs", "kwargs"]
                if parameter["parameter_type"] not in valid_parameter_types:
                    return False, f"Invalid parameter_type {parameter['parameter_type']}. Must be one of: {valid_parameter_types}"
                
                # Validate required field based on parameter_type and default_value
                if parameter["parameter_type"] in ["varargs", "kwargs"]:
                    pass
                    # if parameter["required"] != False:
                    #     return False, "Variadic parameters (*args, **kwargs) must have required: false"
                elif parameter["default_value"] is not None:
                    if parameter["required"] != False:
                        return False, "Parameters with default values must have required: false"
            
            if not isinstance(response["return"], dict):
                return False, "Return must be a dict"
            if "example" not in response["return"]:
                return False, "Required key example not found in return"
            if "type" not in response["return"]:
                return False, "Required key type not found in return"
            if "description" not in response["return"]:
                return False, "Required key description not found in return"
            
            # Skip type validation for special return types (if any)
            if response["return"]["type"] not in AVAILABLE_INSTANCE_TYPES.keys():
                return False, f"Invalid type {response['return']['type']} in return: {list(AVAILABLE_INSTANCE_TYPES.keys())}."
            
            # Skip example type checking for variadic parameter types in return (shouldn't occur, but just in case)
            if response["return"]["type"] not in ["varargs", "kwargs"]:
                if (response["return"]["type"] == "tuple" or response["return"]["type"] == "dict") and not isinstance(response["return"]["example"], str):
                    return False, "Example for tuple or dict must be a wrapped with \"\""
                
                if response["return"]["type"] not in ["tuple", "dict", "any"] and not isinstance(response["return"]["example"], AVAILABLE_INSTANCE_TYPES[response["return"]["type"]]):
                    return False, f"type mismatch with example: {str(type(response['return']['example']))} != {response['return']['type']}"

            if not isinstance(response["exceptions"], list):
                return False, "Exceptions must be a list"

            for exception in response["exceptions"]:
                if "type" not in exception:
                    return False, "Required key type not found in exceptions"
                if "error_message" not in exception:
                    return False, "Required key error_message not found in exceptions"
                if "explictely_defined_from_instruction" not in exception:
                    return False, "Required key explictely_defined_from_instruction not found in exceptions"
                if "description" not in exception:
                    return False, "Required key description not found in exceptions"
        
        return True, ""

    def replace_examples_with_mock_value(type, example):
        import ast
        ...
        if type == "tuple":
            if isinstance(example, str):
                try:
                    ...
                    return ast.literal_eval(example)
        ...
        if type == "dict":
            if isinstance(example, str):
                try:
                    ...
                    # If parsing fails, return a default dict
                    return {"key": "value"}
            return example
            
        if isinstance(example, str):
            return "exmple string"
    ...
    def filter_response(responses: list) -> list:
        if not isinstance(responses, list):
            return []
        ...
        for response in responses:
            ...
            if "exceptions" in response_copy and isinstance(response_copy["exceptions"], list):
                # Use list comprehension to filter exceptions
                response_copy["exceptions"] = [
                    exception for exception in response_copy["exceptions"]
                    if (isinstance(exception, dict) and 
                        exception.get("explictely_defined_from_instruction") == True)
                ]

            for parameter in response_copy["parameters"]:
                if "example" in parameter:
                    parameter["example"] = replace_examples_with_mock_value(parameter["type"], parameter["example"])
            if "return" in response_copy and "example" in response_copy["return"]:
                response_copy["return"]["example"] = replace_examples_with_mock_value(response_copy["return"]["type"], response_copy["return"]["example"])
            
            filtered_responses.append(response_copy)
        
        return filtered_responses
    retry = 0
    code_skeleton = get_code_skeleton()
    messages = [
        {"role": "system", "content": FUNCTION_METADATA_GENERATION_SYSTEM_PROMPT},
        {"role": "user", "content": f"{problem_statement}\n# Code Skeleton:\n{code_skeleton}\n# Important Rules\n- **Do not use list** for multi-line strings if they are not explicitely formatted or defined as list in the instruction or in the code skeleton, use just string type."}
    ]
    while retry < 10:
        try:
            ...
            if is_valid:
                return filter_response(json_response)
            else:
...
def process_create_task(input_dict):
    ...
    original_problem_statement = input_dict.get("problem_statement", "")
    problem_statement, markup_applied = post_process_instruction(original_problem_statement)
    
    if markup_applied:
        function_metadata = generate_function_metatdata(problem_statement)
    else:
        function_metadata = None

    code_skeleton = get_code_skeleton()
    initial_solution = generate_initial_solution(problem_statement, code_skeleton, function_metadata)
    extract_and_write_files(initial_solution)

    test_code = generate_test_code(problem_statement, code_skeleton, function_metadata)

    with open("custom_tests.py", "w") as f:
        f.write(test_code)
...
def fix_task_solve_workflow(problem_statement: str, *, timeout: int, run_id_1: str, \
    test_runner: str = "pytest", test_runner_mode: str = "FILE", n_max_steps = MAX_FIX_TASK_STEPS, model: str = GLM_MODEL_NAME) -> str:
    global run_id
    run_id=run_id_1
    ...
    start_time = time.time()
    logger.info("Starting fix_task_solve_workflow")
    logger.info(f"Working directory: {os.getcwd()}")
    logger.info(f"Max steps: {n_max_steps}, Timeout: {timeout}s")
    logger.info(f"Starting model: {model}")
    step_counter = 0
    start_over_count = 0
    max_start_overs = 2  # Maximum number of automatic start-overs allowed
    
    # Model switching for create task (when n_max_steps < MAX_FIX_TASK_STEPS)
    current_model = model
    current_model_idx = AGENT_MODELS.index(model) if model in AGENT_MODELS else 0
    model_switched = False
    
    for step in range(n_max_steps):
        step_counter += 1
        
        # Switch model at step 50 for create tasks (when n_max_steps < MAX_FIX_TASK_STEPS)
        if step_counter == 50 and n_max_steps < MAX_FIX_TASK_STEPS and not model_switched:
            current_model_idx = (current_model_idx + 1) % len(AGENT_MODELS)
            current_model = AGENT_MODELS[current_model_idx]
            model_switched = True
            logger.warning(f"Step {step_counter}: Switching to model {current_model} for better performance")
        
        # Log progress every 10 steps
        if step_counter % 10 == 0:
            elapsed = time.time() - start_time
            logger.info(f"Progress: Step {step_counter}/{n_max_steps} | Elapsed: {elapsed:.1f}s | Remaining: {timeout - elapsed:.1f}s")
        
        # Check if we need to automatically start over after 100 steps
        if step_counter >= 100 and start_over_count < max_start_overs:
            logger.warning(f"Step {step_counter}: Triggering automatic start over (attempt {start_over_count + 1}/{max_start_overs})")
            os.system("git reset --hard")
            start_over_count += 1
            step_counter = 0
            model_switched = False  # Reset model switch flag for fresh start
            
            # Add notification to COT
            start_over_message = f"Automatic start over triggered after 100 steps (attempt {start_over_count}/{max_start_overs}). Codebase reverted to initial state."
            cot.add_action(EnhancedCOT.Action(
                next_thought=start_over_message,
                next_tool_name="start_over",
                next_tool_args={},
                observation="Done, codebase reverted to initial state. Starting fresh approach.",
                is_error=False,
                inference_error_counter={},
                request_data=[]
            ))
            logger.info("Codebase reverted, continuing with fresh approach")
            continue
        
        if time.time() - start_time > timeout:
            logger.error(f"Global timeout reached at step {step_counter} after {time.time() - start_time:.1f}s")
        ...
        try:
            ...
            if next_tool_name in ["get_approval_for_solution", "apply_code_edit", "finish", "start_over"]:
                logger.info(f"Step {step_counter}: Executing {next_tool_name}")
            ...
            # Log success of important tools
            if next_tool_name == "get_approval_for_solution":
                logger.info(f"Solution approved at step {step_counter}")
            elif next_tool_name == "apply_code_edit":
                logger.info(f"Code edit applied at step {step_counter}")
                
        except EnhancedToolManager.Error as e:
            import traceback  # Ensure traceback is accessible
            error_msg=f"observation: {e.message}"
            logger.error(f"Step {step_counter}: Tool {next_tool_name} failed - {e.error_type}")
            ...
            logger.error(f"Step {step_counter}: Unexpected error in {next_tool_name}")
            cot.add_action(EnhancedCOT.Action(next_thought=next_thought,next_tool_name=next_tool_name,next_tool_args=next_tool_args,observation=error_msg,is_error=True,raw_response=raw_text,total_attempts=total_attempts,inference_error_counter=error_counter,request_data=messages))
            continue
        
        if next_tool_name == "finish":
            logger.info(f"Workflow finished successfully at step {step_counter}")
            break
    else:
        logger.warning(f"Reached maximum steps limit ({n_max_steps})")
    ...
    if not patch or patch.strip() == "":
        logger.error(f"Empty patch generated after {step_counter} steps - no source code changes were made")
        logger.error("This usually means the agent only used test generation tools without actually fixing the source code")
    else:
        logger.info(f"Workflow completed with {len(patch)} characters in patch after {step_counter} steps")
