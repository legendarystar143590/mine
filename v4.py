...
"""
You are an expert testcases reviewer. Your task is to check if the test cases are valid and find missing test cases and edge cases based on the problem statement. 

Important Checks:
1. Check for incorrect/invalid input/output pairs based on the problem statement and fix them or remove if impossible to fix
2. Check if testcases are not covering critical edge cases for the problem statement and add missing testcases
3. Verify all test assertions are correct and logical
4. Ensure tests cover: normal cases, edge cases, boundary conditions, exception scenarios
5. Minimize testcases - keep only essential ones due to context limits

CRITICAL: You MUST respond with ONLY a valid JSON object. No other text, explanations, or markdown.

If tests are PERFECT (no issues found):
{"status": "perfect", "message": "All test cases are valid and comprehensive"}

If tests need UPDATES (issues found or missing cases):
{"status": "updated", "issues_found": ["issue1", "issue2"], "improvements_made": ["fix1", "fix2"], "test_code": "test_file.py\\nimport pytest\\n\\ndef test_function():\\n    assert True\\n"}

RULES:
- Output ONLY valid JSON, nothing else
- No markdown code blocks, no explanations outside JSON
- If status is "updated", test_code field MUST contain the complete test code
...
<testing_rules>
  ...
  <rule>Note that current test functions should be passed originally and generated test function is to catch the issues and edge cases addressed in problem statement and not to pass</rule>
</testing_rules>
## Multi-file awareness (critical):
- Tests and patch contexts may span multiple files. Do not stop after the first similar match or applied fix.
- Keep searching the repository after each match and apply consistent changes to every relevant file before finishing.
...
class EnhancedNetwork:
    ...
    def make_request(cls, messages: list, model: str, attempt: int = 0, temperature: float = 0.0, max_retries: int = 5) -> str:
        ...
        for retry_attempt in range(max_retries):
            try:
                print(f"[REQUEST] run_id: {run_id}, attempt: {retry_attempt + 1}/{max_retries}")
                ...
                try:
                    response = requests.post(url, json=request_data, timeout=600, headers=headers)
                    response.raise_for_status()
                except requests.exceptions.Timeout:
                    if retry_attempt < max_retries - 1:
                        delay = 2 ** retry_attempt  # Exponential backoff: 1, 2, 4, 8, 16 seconds
                        logger.warning(f"Request timeout for model {model}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"Request timeout after {max_retries} attempts for model {model}")
                        return f"ERROR: Request timeout for model {model}"
                except requests.exceptions.ConnectionError as e:
                    if retry_attempt < max_retries - 1:
                        delay = 2 ** retry_attempt
                        logger.warning(f"Connection error for model {model}: {e}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"Connection failed after {max_retries} attempts for model {model}: {e}")
                    ...
                    if e.response.status_code == 429 or (500 <= e.response.status_code < 600):
                        if retry_attempt < max_retries - 1:
                            delay = 2 ** retry_attempt
                            logger.warning(f"HTTP error {e.response.status_code} for model {model}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                            time.sleep(delay)
                            continue
                    ...
                    if retry_attempt < max_retries - 1:
                        delay = 2 ** retry_attempt
                        logger.warning(f"Request error for model {model}: {e}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"Request failed after {max_retries} attempts for model {model}: {e}")
                ...
                try:
                    ...
                    if retry_attempt < max_retries - 1:
                        delay = 2 ** retry_attempt
                        logger.warning(f"Invalid JSON response for model {model}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"Invalid JSON response after {max_retries} attempts for model {model}: {e}")
                ...
                try:
                    ...
                    logger.info(f"✓ Request successful for model {model} on attempt {retry_attempt + 1}")
                    return raw_text
                    
                except (KeyError, IndexError, TypeError) as e:
                    if retry_attempt < max_retries - 1:
                        delay = 2 ** retry_attempt
                        logger.warning(f"Error parsing response structure for model {model}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                        logger.warning(f"Response JSON: {response_json}")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"Invalid response structure after {max_retries} attempts for model {model}: {e}")
                ...
                if retry_attempt < max_retries - 1:
                    delay = 2 ** retry_attempt
                    logger.warning(f"Unexpected error for model {model}: {e}, retrying in {delay}s... (attempt {retry_attempt + 1}/{max_retries})")
                    logger.warning(f"Traceback: {traceback.format_exc()}")
                    time.sleep(delay)
                    continue
                else:
                    logger.error(f"Unexpected error after {max_retries} attempts for model {model}: {e}")
                    logger.error(f"Traceback: {traceback.format_exc()}")
                    return f"ERROR: Unexpected error for model {model}"
        
        # Should never reach here, but just in case
        logger.error(f"Failed to get response after {max_retries} retries for model {model}")
        return f"ERROR: Failed after {max_retries} retries for model {model}"
...
def parse_testcase_validation_response(response: str) -> Optional[dict]:
    ...
    try:
        ...
        if not cleaned.startswith('{'):
            # Try to extract JSON object from response using more flexible regex
            import re
            ...
            if json_match:
                ...
                # Try a simpler approach - find first { to last }
        ...
        try:
            ...
            fixed_response = re.sub(r',\s*}', '}', cleaned)
...
def generate_testcases_with_multi_step_reasoning(problem_statement: str, files_to_test: str, code_skeleton: str) -> str:
    ...
    test_generation_messages = [
        ...
        {
            "role": "user",
            "content": f"Problem Statement:\n{problem_statement}\n\nFiles To Test: {files_to_test}\n\nCode skeleton: \n{code_skeleton}\n\nGenerate the complete and correct testcases in python files.\n\nSTRICT REQUIREMENT: 1. You **MUST** output the **file name** along with file content.\nexample:\n```python\ntest_a.py\ncontents of test_a.py\n\ntest_b.py\ncontents of test_b.py\n```\n\n 2. You must find all test cases of the problem statement and generate the test cases in python files. \n - Must include all test cases necessary for the problem statement.\n - Must find all edge cases and boundary cases. 3. Before making test cases, think about the characteristics of the problem statement and generate the test cases accordingly."
        }
    ]
    while retry < 10:
        try:
            testcode_response = EnhancedNetwork.make_request(test_generation_messages, model=QWEN_MODEL_NAME, temperature=0.0)
            ...
            testcases_check_messages = [
                ...
                {
                    "role": "user",
                    "content": f"Problem statement: {problem_statement}\n\nFiles To Test: {files_to_test}\n\nCode skeleton: \n{code_skeleton}\n\nGenerated Test Code:\n{testcode_response}\n\nAnalyze and validate the test code. You MUST respond with ONLY a valid JSON object. No other text.\n\nIf perfect: {{\"status\": \"perfect\", \"message\": \"All tests are comprehensive\"}}\nIf needs updates: {{\"status\": \"updated\", \"issues_found\": [\"issue1\"], \"improvements_made\": [\"fix1\"], \"test_code\": \"test.py\\nimport pytest\\n...\"}}"
                }   
            ]
            
            testcode_checked_response = EnhancedNetwork.make_request(testcases_check_messages, model=GLM_MODEL_NAME, temperature = 0.1)
            ...
            if validation_result is None:
                ...
                # Check if response looks like test code (starts with .py filename)
                lines = testcases.split("\n")
                if lines and lines[0].endswith(".py"):
                    logger.info("Testcase generation completed with traditional validation")
                    return testcases
                else:
                    # If it doesn't look like test code, retry with more explicit instructions
                    retry += 1
                    test_generation_messages.append({"role": "assistant", "content": testcode_checked_response})
                    test_generation_messages.append({"role": "user", "content": f"JSON parsing failed. Please respond with test code in this exact format:\n```python\ntest_a.py\ncontents of test_a.py\n\ntest_b.py\ncontents of test_b.py\n```"})
                    print(f"Retrying because response doesn't contain valid test code:\n {testcases[:200]}...")
                    continue
                ...
                for validation_iteration in range(max_validation_iterations):
                    ...
                    try:
                        ...
                        testcases_recheck_messages = [
                            ...
                            {
                                "role": "user",
                                "content": f"Problem statement: {problem_statement}\n\nFiles To Test: {files_to_test}\n\nCode skeleton: \n{code_skeleton}\n\nGenerated Test Code:\n{current_testcases}\n\nAnalyze and validate. You MUST respond with ONLY a valid JSON object. No other text.\n\nIf perfect: {{\"status\": \"perfect\", \"message\": \"All tests are comprehensive\"}}\nIf needs updates: {{\"status\": \"updated\", \"issues_found\": [\"issue1\"], \"improvements_made\": [\"fix1\"], \"test_code\": \"test.py\\nimport pytest\\n...\"}}"
                            }   
                        ]
                        
                        revalidation_response = EnhancedNetwork.make_request(testcases_recheck_messages, model=GLM_MODEL_NAME, temperature=0.1)
    ...
    while retry < 10:
        try:
            ...
            if testcases:
                ...
                response = EnhancedNetwork.make_request(messages, model=QWEN_MODEL_NAME, temperature=1.0)
...
def process_create_task(input_dict):
    ...
    initial_solution = FS_initial_solution(problem_statement, code_skeleton)
...
def FS_initial_solution(problem_statement: str, code_skeleton: str) -> str:
    global libraries
    ...
    code_generation_messages = [
        ...
        {
            "role": "user",
            "content": f"Problem Statement:\n{problem_statement}\n\nInitial python files:\n{code_skeleton}\n\nGenerate the complete and correct implementation in python files.\n\nSTRICT REQUIREMENT: \n You **MUST** output the **file name** along with file1 content.\nexample:\n```python\na.py\ncontents of a.py\n\nb.py\ncontents of b.py\n```"
        }
    ]
    code_response = EnhancedNetwork.make_request(code_generation_messages, model=QWEN_MODEL_NAME, temperature=0.3)
