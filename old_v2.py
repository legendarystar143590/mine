...
REJECTION_FEEDBACK_PROMPT = "ERROR: Reject tool call - this exact tool call with same arguments was already attempted {{consecutive_rejections}} times. You're trying the same tool {{next_tool_name}} with identical arguments. This suggests you may be stuck in a loop. Please try a different approach:\n" \
...
```
3. Use only standard Python (no external libraries).
5. Only respond with python code.
"""
)
...
You are working within a Python code repository. Your current directory is the repository root. You will receive a problem statement and must make precise, maintainable code changes that fix the issue fully and pass all tests.

CRITICAL: Follow the step-by-step workflow below EXACTLY. Each step has specific context, rules, and tools you MUST use. Do not skip steps or combine them.
</role>

<step_by_step_workflow>
## STEP 1: INITIAL EXPLORATION AND PROBLEM UNDERSTANDING
**Context**: You need to understand the problem and explore the codebase structure
**Rules**: 
- Start with broad exploration before diving into specifics
- Use multiple search strategies to avoid getting stuck
- Document your findings clearly

**Required Actions**:
1. Use `search_in_all_files_content` to find files related to the problem domain
2. Use `get_file_summary` to understand file purposes and relationships
3. Use `get_file_content` to read key files and understand the codebase structure

**Tools to Use**: `search_in_all_files_content`, `get_file_summary`, `get_file_content`
**Success Criteria**: You have a clear understanding of the codebase structure and can identify relevant files

---

## STEP 2: IMPACT ANALYSIS AND DEPENDENCY MAPPING
**Context**: Before making changes, understand all affected code and dependencies
**Rules**:
- Search globally for all usages of affected APIs
- Trace call chains and data flows
- Identify all files that need updates
- Keep DRY and SOLID principles in mind
**Required Actions**:
1. Use `find_import_usages` to find all places where affected modules are imported
2. Use `find_class_children` and `find_method_overrides` to understand inheritance
3. Use `localize_symbol` to find all usages of specific symbols
4. Use `get_functions` and `get_classes` to understand current patterns
5. Use `get_file_content` to read key files and understand the codebase structure
**Tools to Use**: `find_import_usages`, `find_class_children`, `find_method_overrides`, `localize_symbol`, `get_functions`, `get_classes`, `get_file_content`
**Success Criteria**: You have a complete map of all code that needs to be updated

---

## STEP 3: SOLUTION DESIGN AND PLANNING
**Context**: Design your solution before implementing it
**Rules**:
- Consider multiple solution strategies
- Plan for edge cases and error handling
- Ensure architectural consistency
- Keep DRY and SOLID principles in mind
- Understand how target elements are used in the codebase and find the best solution to fix the problem

**Required Actions**:
1. Analyze the current code structure and design patterns
2. Plan your changes to maintain consistency
3. Consider exception handling requirements
4. Plan test coverage for your changes

**Tools to Use**: `get_file_summary`, `get_functions`, `get_classes` (for understanding current patterns)
**Success Criteria**: You have a clear plan for implementing the fix

---

## STEP 4: CODE IMPLEMENTATION
**Context**: Implement your planned solution
**Rules**:
- Make minimal, targeted changes
- Follow existing code style and patterns
- Implement proper exception handling
- Use appropriate tools for different types of changes

**Required Actions**:
1. Use `apply_code_edit` for search-and-replace operations
2. Use `save_file` for creating new files if needed
3. Use `generate_test_function` to add test coverage

**Tools to Use**: `apply_code_edit`, `save_file`, `generate_test_function`
**Success Criteria**: Your code changes are implemented correctly

---

## STEP 5: VALIDATION AND TESTING
**Context**: Verify that your changes work correctly and don't break the existing code and tests.
**Rules**:
- Run tests to verify functionality
- Check for syntax errors
- Validate that all affected code works together
- Ensure all possible positive and negative test cases are covered based on the problem statement
- Cover all edge cases 
**Required Actions**:
1. Use `run_code` to test specific code snippets
2. Use `run_repo_tests` to run the full test suite
3. Check for any syntax errors in modified files

**Tools to Use**: `run_code`, `run_repo_tests`
**Success Criteria**: 
1. Generate the test script to catch the problem described in the problem statement and check if the solution works correctly
2. The solution does not break the existing code and tests
3. Ensure both problem was solved by testing with test script and solution does not break the existing code and tests
---

## STEP 6: FINAL VERIFICATION AND CLEANUP
**Context**: Ensure everything is working and clean up any issues
**Rules**:
...
**Required Actions**:
1. Run final tests to confirm everything works
2. Review your changes for code quality
3. Use `get_approval_for_solution` if manual review is needed
4. Use `finish` to complete the task

**Tools to Use**: `run_repo_tests`, `get_approval_for_solution`, `finish`
**Success Criteria**: The problem is completely solved and all tests pass
</step_by_step_workflow>

<exception_handling_rules>
## Exception Handling Requirements (Apply in Step 3):

- Implement focused and specific exception handling to catch anticipated error types
- Use multiple `except` blocks when different exception types require distinct handling logic
- Keep `try` blocks small and surround only code that might raise specific exceptions
- Avoid catching generic `Exception` or bare `except:` unless re-raising or logging
- Raise custom exceptions or propagate original exceptions properly, preserving stack trace
- Use `try-except-else-finally` structure where appropriate
- Log error conditions appropriately with meaningful messages
- Handle all expected edge cases without suppressing unknown exceptions
- Use `raise ... from` syntax to maintain exception chaining
- Ensure exception handling maintains backward compatibility
</exception_handling_rules>

<multi_file_coordination_rules>
## Multi-file Coordination Requirements (Apply in Steps 2-4):

- Always extend impact analysis beyond the first file or function fixed
- Use global search capabilities to uncover hidden dependencies
- Work systematically until all related usages and overrides are updated simultaneously
- Coordinate updates across all modules that use related functionalities
- Verify changes in one module are compatible with dependent modules
</multi_file_coordination_rules>

<testing_validation_rules>
## Testing and Validation Requirements (Apply in Steps 2, 5-6):

- Use test failures as direct guidance on what scenario or behavior the solution must fulfill
- Understand test setup patterns and manipulate metadata or environment states as needed
- Ensure tests cover both expected and edge error cases
- Verify that exception handling does not interfere with test execution
- Generate new targeted tests if additional coverage is needed
</testing_validation_rules>

<code_quality_rules>
## Code Quality Requirements (Apply in Steps 3-4):

- Think holistically about system design idioms like layering, dependency injection, fallback hierarchies
- Make code refactors when they increase clarity and robustness over ad hoc patches
- Abstract shared or repeated logic into reusable helper methods or functions
- Enforce consistent reuse to avoid fragmented fixes
- Maintain clear code quality and follow repository style guidelines
- Avoid creating unnecessary files
- Design solutions with modular components to promote reusability
</code_quality_rules>

<error_recovery_rules>
## Error Recovery and Loop Prevention:

- If you get stuck in a loop (same tool called repeatedly), STOP and try a different approach
- If a tool fails multiple times, try alternative tools or strategies
- If search operations fail, try different search terms or use `get_file_content` to explore structure
- Always read error messages carefully and adapt your strategy accordingly
- Use the comprehensive tool execution summary to understand what went wrong
- If you need to start over completely, use `start_over` to revert changes and try a new approach
</error_recovery_rules>

<tools>
## Available Tools (Use appropriate tools for each step):
...
1. **Strict Triplet Format**:
   - `next_thought`: Detailed reasoning (include:
     ...
     - Testing and validation plan
     - Code quality plan
     - Your decision on the next move)
...
class KnowledgeGraph:
    """
    KnowledgeGraph integrated to find relevant files & localize code.
    Uses TF-IDF + keyword extraction + LLM evaluation to choose candidates.
    """
    DEFAULT_EXTENSIONS = ["*.py"]
    def __init__(self, project_path: str = ".", allowed_extensions: list = None, network: EnhancedNetwork = None):
        ...
        self.files = []
        self.test_files = []
        self.network = network or EnhancedNetwork()
        self._build_knowledge_graph_called = False
        ...
        if tree:
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    functions[node.name] = {"name": node.name, "line": node.lineno, "args": [arg.arg for arg in node.args.args]}
                elif isinstance(node, ast.ClassDef):
                    classes[node.name] = {"name": node.name, "line": node.lineno, "bases": [base.id if isinstance(base, ast.Name) else str(base) for base in node.bases]}
                elif isinstance(node, ast.Assign):
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            variables.append(target.id)
        return {"source": source, "imports": imports, "functions": functions, "classes": classes, "variables": variables}
    ...
    def _build_knowledge_graph(self):
        ...
        # Only include Python files and README files
        allowed_patterns = [
            "**/*.py",           # Python files
            "**/README*",        # README files (README, README.md, README.txt, etc.)
            "**/readme*"         # Case-insensitive README files
        ]
        ...
        for pattern in allowed_patterns:
            for filepath in glob.glob(os.path.join(self.project_path, pattern), recursive=True):
                ...
                if any(part in ['venv', '.venv'] for part in path_parts):
                    continue
                try:
                    ...
                    self.knowledge_graph[filepath] = self._parse_file(filepath)
                    file_count += 1
                except Exception as e:
                    logger.warning(f"Error parsing {filepath}: {e}")
        
        self._build_knowledge_graph_called = True
        logger.info(f"KnowledgeGraph parsed {file_count} files (Python + README + test files only)")
        return file_count > 0
    ...
    def _compute_embeddings(self):
        if TfidfVectorizer is None:
            ...
            self.vectorizer = None
            self.test_vectorizer = None
            self.embeddings = None
            self.test_embeddings = None
            return
        
        # Compute embeddings for source files (non-test)
        source_documents = []
        for fp in self.files:
            source_documents.append(self.knowledge_graph[fp]["source"])
        
        # Compute embeddings for test files
        test_documents = []
        for fp in self.test_files:
            test_documents.append(self.knowledge_graph[fp]["source"])
        
        try:
            # Source files embeddings
            if source_documents:
                self.vectorizer = TfidfVectorizer(stop_words="english", max_features=20000)
                self.embeddings = self.vectorizer.fit_transform(source_documents).astype('float32')
                logger.info(f"✅ Computed embeddings for {len(source_documents)} source files")
            else:
                self.vectorizer = None
                self.embeddings = None
            
            # Test files embeddings (separate vectorizer for test-specific vocabulary)
            if test_documents:
                self.test_vectorizer = TfidfVectorizer(stop_words="english", max_features=20000)
                self.test_embeddings = self.test_vectorizer.fit_transform(test_documents).astype('float32')
                logger.info(f"✅ Computed embeddings for {len(test_documents)} test files")
            else:
                self.test_vectorizer = None
                self.test_embeddings = None
    ...
    def localize_code_by_symbol(self, file_path: str, symbol: str) -> Optional[Tuple[int, int]]:
        """Return (start_line, end_line) for a function/class by symbol name using AST in KG data."""
        abs_path = file_path if os.path.isabs(file_path) else os.path.join(self.project_path, file_path)
        if abs_path not in self.knowledge_graph:
            try:
                self.knowledge_graph[abs_path] = self._parse_file(abs_path)
                if abs_path not in self.files:
                    self.files.append(abs_path)
            except Exception:
                return None
        data = self.knowledge_graph.get(abs_path, {})
        funcs = data.get("functions", {})
        classes = data.get("classes", {})
        if symbol in funcs:
            start = funcs[symbol]["line"]
            # best-effort end: find next function/class lineno or EOF
            lines = data.get("source", "").splitlines()
            end = len(lines)
            for node in sorted(list(funcs.values()) + list(classes.values()), key=lambda x: x["line"]):
                if node["line"] > start:
                    end = node["line"] - 1
                    break
            return start, end
        if symbol in classes:
            start = classes[symbol]["line"]
            lines = data.get("source", "").splitlines()
            end = len(lines)
            for node in sorted(list(funcs.values()) + list(classes.values()), key=lambda x: x["line"]):
                if node["line"] > start:
    ...
    def get_project_structure_summary(self) -> str:
        """
        Generate a comprehensive summary of the project structure based on knowledge graph.
        Returns detailed information about files and their components with relationships.
        
        Returns:
            str: Comprehensive project structure analysis as a formatted string
        """
        # Ensure knowledge graph is built
        if not self._build_knowledge_graph_called:
            self._build_knowledge_graph()
        
        if not self.knowledge_graph:
            return "No project structure available - knowledge graph is empty"
        ...
        # Add file breakdown with relationships
        lines.append("### FILE BREAKDOWN WITH RELATIONSHIPS")
        lines.append("")
        
        for filepath in self.files:
            data = self.knowledge_graph.get(filepath, {})
            ...
            if classes:
                ...
                for class_name, class_info in classes.items():
                    ...
                    # Show class methods (functions within this class)
                    class_methods = []
                    for func_name, func_info in functions.items():
                        if func_info.get('line', 0) > class_info.get('line', 0):
                            # This is a rough heuristic - in practice, you might want to parse the AST more carefully
                            class_methods.append(func_name)
                    
                    if class_methods:
                        lines.append(f"    Methods: {', '.join(class_methods)}")
                    
                    # Show class variables (variables that might belong to this class)
                    class_vars = []
                    for var in variables:
                        if var.startswith(class_name.lower()) or var.startswith('_'):
                            class_vars.append(var)
                    
                    if class_vars:
                        lines.append(f"    Variables: {', '.join(class_vars)}")
            ...
            if functions:
                ...
                for func_name, func_info in functions.items():
                    ...
                    # Show function variables (variables that might be related to this function)
                    func_vars = []
                    for var in variables:
                        if var.startswith(func_name.lower()) or var in func_info.get('args', []):
                            func_vars.append(var)
                    
                    if func_vars:
                        lines.append(f"    Related Variables: {', '.join(func_vars)}")
                
                lines.append("")
            
            # Show standalone variables
            if variables:
            ...
            # Show local imports for this file
            ...
            if imports:
                ...
                for imp in imports:
                    ...
                    if '.' in imp and not imp.startswith('.'):
                        ...
                        if any(potential_file in f or f.endswith(potential_file) for f in self.files):
                            local_imports.append(imp)
                    elif not imp.startswith('.'):
                        potential_file = imp + '.py'
                        if any(potential_file in f or f.endswith(potential_file) for f in self.files):
                            local_imports.append(imp)
                    else:
                        local_imports.append(imp)
        ...
        # Add project-wide relationships
        lines.append("### PROJECT-WIDE RELATIONSHIPS")
        lines.append("")
        
        # Collect all unique local imports
        ...
        if all_local_imports:
            lines.append("**Inter-file Dependencies:**")
            for imp in sorted(all_local_imports):
                lines.append(f"  • {imp}")
            lines.append("")
        ...
        test_files = [f for f in self.files if 'test' in os.path.basename(f).lower()]
        if test_files:
            lines.append(f"• Testing: {len(test_files)} test files detected")
        
        config_files = [f for f in self.files if any(ext in f for ext in ['.json', '.yaml', '.yml', '.toml', '.ini', '.cfg'])]
        if config_files:
            lines.append(f"• Configuration: {len(config_files)} configuration files detected")
        
        # File organization
        dirs = set()
        for filepath in self.files:
            dir_path = os.path.dirname(os.path.relpath(filepath, self.project_path))
            if dir_path:
                dirs.add(dir_path)
        
        if len(dirs) > 5:
            lines.append(f"• Modular Structure: {len(dirs)} directories suggest good code organization")
        elif len(dirs) <= 2:
            lines.append("• Flat Structure: Most files in root directory")
        
        return "\n".join(lines)

    def ensure_knowledge_graph_ready(self) -> bool:
        """
        Ensure knowledge graph is built and embeddings are computed.
        
        Returns:
            bool: True if knowledge graph is ready, False otherwise
        """
        try:
            if not self._build_knowledge_graph_called:
                logger.info("Building knowledge graph...")
                self._build_knowledge_graph()
                logger.info(f"Knowledge graph built with {len(self.files)} files")
            ...
            logger.error(f"Failed to prepare knowledge graph: {e}")
            return False
    
    def get_file_detailed_summary(self, file_path: str) -> str:
        """
        Get a detailed, well-structured summary of a file's attributes and structure.
        
        This method generates a concise but comprehensive explanation of what the file does
        based on its imports, classes, functions, and variables. The output is optimized
        to reduce token usage while providing enough context for LLM understanding.
        
        Args:
            file_path: Path to the file (relative or absolute)
            
        Returns:
            str: Detailed structured summary of the file
        """
        # Ensure knowledge graph is built
        if not self._build_knowledge_graph_called:
            self._build_knowledge_graph()
        ...
        if abs_path not in self.knowledge_graph:
            return f"File not found in knowledge graph: {file_path}"
        
        # Get file data
        file_data = self.knowledge_graph[abs_path]
        ...
        imports = file_data.get("imports", set())
        if imports:
            lines.append("## Dependencies (Imports)")
            # Separate standard library from local imports
            std_imports = []
            local_imports = []
            for imp in sorted(imports):
                # Simple heuristic: local imports often have dots or match project structure
                if any(imp in str(f) for f in self.files):
                    local_imports.append(imp)
                else:
            ...
            if local_imports:
                lines.append("**Local Imports:**")
                for imp in local_imports[:10]:  # Limit to 10
                    lines.append(f"  • {imp}")
                if len(local_imports) > 10:
                    lines.append(f"  • ... and {len(local_imports) - 10} more")
            
            if std_imports:
                lines.append("**External Libraries:**")
                for imp in std_imports[:10]:  # Limit to 10
                    lines.append(f"  • {imp}")
                if len(std_imports) > 10:
                    lines.append(f"  • ... and {len(std_imports) - 10} more")
            
            lines.append("")
        
        # 2. Classes (what this file defines)
        classes = file_data.get("classes", {})
        if classes:
            lines.append("## Classes Defined")
            for class_name, class_info in list(classes.items())[:10]:  # Limit to 10
                bases = class_info.get("bases", [])
                if bases:
                    lines.append(f"  • **{class_name}** (inherits: {', '.join(bases)})")
                else:
                    lines.append(f"  • **{class_name}**")
                lines.append(f"    - Defined at line {class_info.get('line', '?')}")
        ...
        # 3. Functions (what this file provides)
        functions = file_data.get("functions", {})
        if functions:
            lines.append("## Functions Defined")
            for func_name, func_info in list(functions.items())[:15]:  # Limit to 15
                args = func_info.get("args", [])
                args_str = f"({', '.join(args)})" if args else "()"
                lines.append(f"  • **{func_name}**{args_str}")
                lines.append(f"    - Line {func_info.get('line', '?')}")
        ...
        # 4. Variables (module-level state)
        variables = file_data.get("variables", [])
        if variables:
            lines.append("## Module Variables")
            for var in variables[:10]:  # Limit to 10
                lines.append(f"  • {var}")
        ...
        # 5. File purpose inference (based on structure)
        lines.append("## Inferred Purpose")
        purpose_hints = []
        
        if any(name.lower() in ['test', 'tests'] for name in [rel_path, *classes.keys(), *functions.keys()]):
            purpose_hints.append("Testing/Test utilities")
        if any(name.lower() in ['model', 'models'] for name in [rel_path, *classes.keys()]):
            purpose_hints.append("Data models/ORM")
        if any(name.lower() in ['view', 'views', 'api'] for name in [rel_path, *functions.keys()]):
            purpose_hints.append("API/View layer")
        if any(name.lower() in ['util', 'utils', 'helper', 'helpers'] for name in [rel_path, *functions.keys()]):
            purpose_hints.append("Utility/Helper functions")
        if any(name.lower() in ['config', 'settings'] for name in [rel_path, *variables]):
            purpose_hints.append("Configuration")
        
        if purpose_hints:
            lines.append(f"  • {', '.join(purpose_hints)}")
        else:
...
class EnhancedNetwork:
    ...
    def make_request(cls,messages:list,model:str,attempt:int=0, temperature:float=0.0)->str:
        ...
        max_retries = 5
        base_delay = 5.0  # Start with 2 seconds delay
        
        for retry_attempt in range(max_retries + 1):
            try:
                response = requests.post(url, json=request_data, timeout=120, headers=headers)
                
                # Check for 500 or 504 errors specifically
                if response.status_code in [500, 504]:
                    if retry_attempt < max_retries:
                        delay = base_delay * (2 ** retry_attempt)  # Exponential backoff
                        error_type = "Server error" if response.status_code == 500 else "Gateway timeout"
                        logger.warning(f"{error_type} {response.status_code} for model {model}, retrying in {delay} seconds (attempt {retry_attempt + 1}/{max_retries + 1})")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"Gateway timeout 504 for model {model} after {max_retries} retries")
                        return f"ERROR: Gateway timeout 504 for model {model} after {max_retries} retries"
                ...
                if e.response.status_code == 500:
                    # This will be handled by the 500 check above
                    continue
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def __init__(self, available_tools: Optional[list[str]] = [], test_runner: str = "pytest", test_runner_mode: str = "FILE"):
        ...
        # Initialize Knowledge Graph for architectural analysis
        self.knowledge_graph = KnowledgeGraph(project_path=".")
    ...
    def _ensure_kg_ready(self):
        """Lazy initialization of knowledge graph"""
        if not self.kg_initialized:
            self.knowledge_graph.ensure_knowledge_graph_ready()
    ...
    def get_file_summary(self, file_path: str) -> str:
        '''
        Get a detailed architectural summary of a file including its classes, functions, imports, and relationships.
        This is more efficient than reading the entire file when you need to understand file structure.
        ...
        Output:
            ...
            - Dependencies (local and external imports)
            - Classes defined (with inheritance info)
            - Functions defined (with signatures)
            - Module variables
            - Inferred purpose
        '''
        self._ensure_kg_ready()
        try:
            summary = self.knowledge_graph.get_file_detailed_summary(file_path)
            return summary
        except Exception as e:
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.FILE_NOT_FOUND.name,
                f"Error getting file summary for {file_path}: {str(e)}"
            )
    
    @EnhancedToolManager.tool
    def find_class_children(self, class_name: str, file_path: str = None) -> str:
        '''
        Find all classes that inherit from a given base class. Essential for multi-file fixes involving inheritance.
        
        Arguments:
            class_name: name of the base class to find children for
            file_path: optional file path where the base class is defined (helps narrow search)
            
        Output:
            List of child classes with their file paths and inheritance information
        '''
        self._ensure_kg_ready()
        try:
            results = []
            ...
            # Search through all files for classes that inherit from the base class
            for filepath in self.knowledge_graph.files:
                data = self.knowledge_graph.knowledge_graph.get(filepath, {})
                classes = data.get("classes", {})
                
                for child_class_name, class_info in classes.items():
                    bases = class_info.get("bases", [])
                    # Check if this class inherits from the target class
                    if class_name in bases or any(class_name in base for base in bases):
                        rel_path = os.path.relpath(filepath, self.knowledge_graph.project_path)
                        results.append(
                            f"  • {child_class_name} in {rel_path} (line {class_info.get('line', '?')})\n"
                            f"    Inherits from: {', '.join(bases)}"
                        )
            
            if not results:
                return f"No child classes found that inherit from {class_name}"
            
            return f"Classes inheriting from {class_name}:\n" + "\n".join(results)
        except Exception as e:
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.UNKNOWN.name,
                f"Error finding child classes: {str(e)}"
            )
    
    @EnhancedToolManager.tool
    def find_method_overrides(self, method_name: str, base_class: str = None) -> str:
        '''
        Find all files where a method is defined or overridden. Critical for ensuring consistency across inheritance hierarchies.
        
        Arguments:
            method_name: name of the method to search for
            base_class: optional base class name to narrow search to its children
            
        Output:
            List of all method implementations with file paths and line numbers
        '''
        self._ensure_kg_ready()
        try:
            results = []
            ...
            for filepath in self.knowledge_graph.files:
                ...
                # Check if method exists in this file
                if method_name in functions:
                    func_info = functions[method_name]
                    rel_path = os.path.relpath(filepath, self.knowledge_graph.project_path)
                    
                    # Get class context if available
                    classes = data.get("classes", {})
                    in_class = "standalone function"
                    for class_name, class_info in classes.items():
                        if func_info.get("line", 0) > class_info.get("line", 0):
                            in_class = f"in class {class_name}"
                            break
                    ...
                    results.append(
                        f"  • {method_name}{args_str} {in_class}\n"
                        f"    File: {rel_path} (line {func_info.get('line', '?')})"
                    )
            
            if not results:
                return f"No implementations found for method '{method_name}'"
            
            return f"Method '{method_name}' found in {len(results)} location(s):\n" + "\n".join(results)
        except Exception as e:
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.UNKNOWN.name,
                f"Error finding method overrides: {str(e)}"
            )
    
    @EnhancedToolManager.tool
    def find_import_usages(self, module_or_symbol: str) -> str:
        '''
        Find all files that import a specific module or symbol. Essential for impact analysis when fixing shared code.
        
        Arguments:
            module_or_symbol: name of the module or symbol to search for (e.g., "validators" or "BaseHandler")
            
        Output:
            List of files that import the module/symbol with import statements
        '''
        self._ensure_kg_ready()
        try:
            results = []
            ...
            for filepath in self.knowledge_graph.files:
                ...
                if module_or_symbol in imports or any(module_or_symbol in imp for imp in imports):
                    rel_path = os.path.relpath(filepath, self.knowledge_graph.project_path)
                    
                    # Find matching imports
                    matching_imports = [imp for imp in imports if module_or_symbol in imp]
                    results.append(
                        f"  • {rel_path}\n"
                        f"    Imports: {', '.join(matching_imports)}"
                    )
            
            if not results:
                return f"No files found that import '{module_or_symbol}'"
            
            return f"Files importing '{module_or_symbol}' ({len(results)} file(s)):\n" + "\n".join(results)
        except Exception as e:
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.UNKNOWN.name,
                f"Error finding import usages: {str(e)}"
            )
    
    @EnhancedToolManager.tool
    def localize_symbol(self, file_path: str, symbol_name: str) -> str:
        '''
        Localize a specific function or class in a file by returning its exact line range.
        More precise than reading the entire file when you know what you're looking for.
        
        Arguments:
            file_path: path to the file containing the symbol
            symbol_name: name of the function or class to localize
            
        Output:
            Line range (start, end) where the symbol is defined, or error if not found
        '''
        self._ensure_kg_ready()
        try:
            result = self.knowledge_graph.localize_code_by_symbol(file_path, symbol_name)
            
            if result is None:
                return f"Symbol '{symbol_name}' not found in {file_path}"
            
            start_line, end_line = result
            return f"Symbol '{symbol_name}' found in {file_path}:\n  Lines {start_line}-{end_line}\n\nUse get_file_content(file_path='{file_path}', search_start_line={start_line}, search_end_line={end_line}) to read it."
        except Exception as e:
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.FILE_NOT_FOUND.name,
                f"Error localizing symbol: {str(e)}"
            )
...
def check_problem_type(problem_statement: str) -> str:
    ...
    while retry < 10:
        try:
            ...
            if PROBLEM_TYPE_CREATE in response:
                response = PROBLEM_TYPE_CREATE
            elif PROBLEM_TYPE_FIX in response:
                response = PROBLEM_TYPE_FIX
...
def generate_test_code(problem_statement: str, code_skeleton: str) -> str:
    retry = 0
...
def process_create_task(input_dict):
    ...
    problem_statement, _ = post_process_instruction(original_problem_statement)
    ...
    test_code = generate_test_code(problem_statement, code_skeleton)
    ...
    patch = fix_task_solve_workflow(
        ...
        run_id_1=run_id,
        instance_id="",
        test_runner=f"python",
        test_runner_mode="FILE",
        n_max_steps=30
    )

    if patch is None:
        print("No patch generated, using initial solution...")
        extract_and_write_files(initial_solution)
