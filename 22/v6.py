...
Analyze test cases and problem statement, generate the solution code to pass all test cases.  
...
"""
You are a top-tier developer focused on protocol excellence and performance optimization. Review generated code to fix protocol violations, performance bottlenecks, and ensure optimal resource utilization across any programming language or system.

PROTOCOL ESSENTIAL CHECKS:

1. ITERATION & TRAVERSAL PROTOCOLS:
   - Ensure iteration mechanisms work correctly and efficiently
   - Verify proper termination conditions and state advancement
   - Check that all elements are visited without skips or repeats
   - Optimize traversal algorithms for performance
   - Validate iterator state management and cleanup

2. EVENT & CALLBACK MANAGEMENT:
   - Fire events/callbacks only on actual state changes
   - Ensure proper event registration and deregistration
   - Provide callbacks with correct parameters and context
   - Prevent memory leaks from event handler references
   - Optimize callback execution performance

3. STATE MANAGEMENT & CONSISTENCY:
   - Initialize and maintain internal state correctly
   - Ensure all operations update relevant state variables
   - Maintain consistent state across all operations
   - Prevent orphaned objects or memory leaks
   - Optimize state update operations for performance

4. RESOURCE MANAGEMENT:
   - Proper allocation and deallocation of resources
   - Efficient memory usage and garbage collection
   - Connection pooling and resource reuse
   - Prevent resource leaks and exhaustion
   - Optimize resource access patterns

5. PERFORMANCE OPTIMIZATION:
   - Minimize unnecessary computations and operations
...
DETAILED FIXES:
- If iteration is incomplete: Ensure all elements are processed
- If state updates are missing: Verify all operations update relevant state
- If resource cleanup is missing: Add proper cleanup mechanisms
- If performance is suboptimal: Optimize algorithms and data access
- If protocols are violated: Implement proper interface compliance

VERIFICATION DETAILS:
✓ Check all interfaces implement required methods correctly
✓ Verify state updates occur in every relevant operation
✓ Ensure proper resource cleanup and memory management
✓ Validate performance characteristics and optimization opportunities
✓ Confirm protocol compliance and interface adherence

If no issues: Return code unchanged.

STRICT and MANDATORY: Only return the final corrected code. Don't elaborate or provide explanations.
"""
)

FINAL_CORRECTNESS_CHECK_PROMPT = textwrap.dedent(
"""
You are an ultimate code expert focusing on absolute correctness validation. Detect and fix algorithmic, semantic, edge-case, and specification compliance issues across any programming language or problem domain.

CORRECTNESS MASTER CHECKS:

1. ALGORITHM CHECKS:
   - Verify all operations produce mathematically/logically correct results
   - Ensure data transformations maintain integrity
   - Validate sorting, filtering, and mapping operations
   - Check arithmetic and computational accuracy
   - Verify search and retrieval operations
   - Ensure proper data structure manipulations

2. SEMANTIC VALIDATION:
   - Functions/methods do exactly what their names and documentation promise
   - Return types match expected interfaces and contracts
   - Side effects are consistent and documented
   - API contracts are strictly honored
   - Behavior matches specifications and requirements

3. EDGE CASES & BOUNDARY CONDITIONS:
   - Handle empty/null inputs gracefully
   - Manage single element and minimal cases
   - Test boundary values and limits
   - Detect boundary errors
   - Prevent division by zero and overflow errors
   - Handle extreme values and edge conditions

4. STRICT SPECIFICATION ADHERENCE:
   - Review problem statement and requirements meticulously
   - Implement every required behavior and feature
   - Ensure all examples and test cases work correctly
   - Avoid missing or misunderstanding any requirements
   - Validate against all given constraints

5. STRUCTURAL & LOGICAL INVARIANTS:
   - Maintain data structure integrity throughout operations
   - Ensure proper object relationships and references
   - Validate state transitions and lifecycle management
   - Check resource management and cleanup
   - Verify concurrency and thread safety where applicable

MASTER FIX INSTRUCTIONS:
1. Build comprehensive mental models of expected outcomes
2. Validate semantics: Code does exactly what it promises
3. Test ALL corner cases: Empty inputs, boundary values, error conditions
4. Examine logic validity: Ensure conditions, loops, and invariants are correct
5. Confirm structural integrity during all operations
6. Verify performance characteristics and resource usage

If no problems are found: Return the code unchanged.
...
You are an expert Python pytest testcase developer. Your task is to generate a complete testcases for the given problem statement.
...
5. Generate the test code based on the project structure and problem statement.
6. Use pytest-style test functions (def test_*) instead of unittest classes.
...
4. Use pytest assertions (assert statements) instead of unittest assertions.
...
"""
You are an expert Python pytest testcase developer. 
    Important points:-
    ...
    - Use pytest-style test functions instead of unittest classes.
    ...
    def test_main_func():
        assert main_func() == "expected_output"

    def test_edge_case():
        assert main_func("edge_input") == "expected_edge_output"
"""
)

TESTCASES_CHECK_PROMPT = textwrap.dedent(
"""
You are an expert pytest testcases reviewer specializing in invalid testcases detection and prevention. Your task is to analyze the generated test code if it's all valid for the problem statement.
...
4. Ensure all test functions use pytest-style (def test_*) instead of unittest classes
5. Use pytest assertions (assert statements) instead of unittest assertions
...
# Hey there! You're a Coding Assistant 🚀. I have uploaded all files of a python repository. Your current working directory is at the root of that repo. You will be provided with a problem statement and test files to implement the solution.

<workflow>
## Follow these steps to implement the solution:
1. **Analyze the problem statement**: Understand what needs to be implemented, including all requirements, constraints, and expected behavior.
2. **Validate the test files**: Review the provided test files to check all are correct based on the project structure and problem statement. Never edit/modify the existing test cases.  
3. **Examine the test files**: Review the provided test files to understand the expected interface, function signatures, and test cases that your implementation must pass.
4. **Find relevant files**: Use `search_in_all_files_content` to locate existing code files that need to be modified or where new code should be added.
5. **Understand the codebase structure**: Use `get_file_content`, `get_functions`, and `get_classes` to understand the existing code structure and patterns.
6. **Implement the solution**: Create or modify the necessary files to implement the required functionality.
7. **Validate against tests**: Run the provided test files using `run_repo_tests` to ensure your implementation passes all test cases.
8. **Handle edge cases**: Think about edge cases and ensure your implementation handles them correctly.
9. **Code quality**: Ensure all variables, functions and class meet the the requirements of <principles> 
10. **Final validation**: Run the provided test files using `run_repo_tests` to ensure your implementation passes all test cases.
11. **Iterate step 4-10 until all test files pass**
12. All passed, call finish tool. 
<workflow>

<guidelines>
## Important guidelines:
- **Test-driven development**: Use the provided test files as your primary guide for what needs to be implemented.
- **Interface compliance**: Ensure your implementation matches the exact function signatures and behavior expected by the tests.
- **Test file validation**: If test files cannot be executed due to missing imports, incorrect paths, or structural issues, you may need to update them to be executable while preserving all test cases.
- **Minimal changes**: Only create or modify files that are absolutely necessary for the solution.
- **Error handling**: Implement proper error handling and edge case management as expected by the tests.
- **Dependencies**: If you encounter missing dependencies while running tests, note this limitation as you don't have internet access.
<guidelines>

## Test execution guidance:
- **Primary method**: Use `run_repo_tests` with the provided test files to validate your implementation (uses pytest by default).
- **Fallback method**: If `run_repo_tests` fails (e.g., test runner not found), use `run_code` to execute tests directly:
  ```python
  import pytest
  import sys
  sys.path.append(".")
  ...
  exit_code = pytest.main(["-v", "test_file_name.py"])
  print(f"Pytest exit code: {exit_code}")
  # Exit code 0 means all tests passed
  ```
- **Test validation**: 
  - For pytest: Exit code 0 means all tests passed
  - Look for "PASSED" in output for successful tests
  - Look for "FAILED" or "ERROR" in output for failed tests
- **Iterative approach**: If tests fail, analyze the failure output carefully to understand what needs to be fixed and iterate on your implementation.
- **Error handling**: If you encounter errors while running tests, try alternative approaches (like using `run_code` instead of `run_repo_tests`).
- **Consistency check**: If tests pass but you're unsure, run them multiple times to ensure consistency.
...
# Hey there! You're a Coding Assistant :rocket:. I have uploaded all files of a python repository. Your current working directory is at the root of that repo. You will be provided with a problem statement and you need to make the necessary changes to fix the issue.
    ...
    def tool(fn):
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                # Catch any other exceptions and return them as error messages
                logger.error(f"Unexpected error in tool {fn.__name__}: {str(e)}")
                self.tool_failure[fn.__name__][EnhancedToolManager.Error.ErrorType.UNKNOWN]+=1
                return f"Error in {fn.__name__}: {str(e)}"
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def _get_file_content(self,file_path: str, search_start_line: int = None, search_end_line: int = None, search_term: str = None,limit:int=5000)->str:
        try:
            ...
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.FILE_NOT_FOUND,f"Error: File '{file_path}' not found.")
        except PermissionError:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.FILE_NOT_FOUND,f"Error: Permission denied accessing file '{file_path}'.")
        except UnicodeDecodeError as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.FILE_NOT_FOUND,f"Error: Cannot decode file '{file_path}' as UTF-8. {str(e)}")
        except Exception as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.UNKNOWN,f"Error reading file '{file_path}': {str(e)}")
    ...
    def start_over(self,problem_with_old_approach:str,new_apprach_to_try:str):
        ...
        try:
            ...
            result = subprocess.run(["git", "reset", "--hard"], capture_output=True, text=True, timeout=30)
            if result.returncode != 0:
                raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.GIT_OPERATION_FAILED,f"Error reverting changes: {result.stderr}")
            ...
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.TIMEOUT,f"Error reverting changes: Timeout after 30 seconds.")
        except subprocess.SubprocessError as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.GIT_OPERATION_FAILED,f"Error reverting changes: {str(e)}")
        except Exception as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.UNKNOWN,f"Error reverting changes: {str(e)}")
    ...
    def run_repo_tests(self,file_paths:List[str])->str:
        ...
        try:
            ...
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.TIMEOUT,f"Error running tests: Timeout after 90 seconds. {str(e)}")
        except subprocess.SubprocessError as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.RUNTIME_ERROR,f"Error running tests: Subprocess error. {str(e)}")
        except Exception as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.UNKNOWN,f"Error running tests: Unexpected error. {str(e)}")
    ...
    def run_code(self,content:str,file_path:str)->str:
        ...
        try:
            ...
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.TIMEOUT,f"Error running code: Timeout after 60 seconds. {str(e)}")
        except subprocess.SubprocessError as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.RUNTIME_ERROR,f"Error running code: Subprocess error. {str(e)}")
        except Exception as e:
            raise EnhancedToolManager.Error(EnhancedToolManager.Error.ErrorType.UNKNOWN,f"Error running code: Unexpected error. {str(e)}")
        ...
        try:
            ...
            if testcases:
                ...
                messages = [
                    ...
                    {
                        "role": "user",
                        "content": f"""Problem Statement:\n{problem_statement}\n\nCode skeleton: \n{code_skeleton}\n\nGenerate the ground truth and all edge case coveraging testcases."""
                    }
                ]
...
def process_create_task(input_dict):
    ...
    # Step 1: Generate test cases first (test-driven approach)
    logger.info("Step 1: Generating test cases first...")
    ...
    # Step 2: Generate initial solution based on test cases
    logger.info("Step 2: Generating initial solution based on test cases...")
    initial_solution = FS_initial_solution_with_tests(problem_statement, code_skeleton, test_cases)
    created_files = extract_and_write_files(initial_solution)

    timeout = DEFAULT_TIMEOUT - (time.time()-start_time) - 60
    
    patch = create_task_solve_workflow_fix(
        problem_statement,
        temperature=0.0,
        timeout=timeout,
        run_id_1=run_id,
...
def FS_initial_solution_with_tests(problem_statement: str, code_skeleton: str, test_cases: str) -> str:
    """
    Generate initial solution based on problem statement, code skeleton, and test cases.
    This follows a test-driven development approach where the solution is designed to pass the tests.
    """
    code_generation_messages = [
        ...
        {
            "role": "user",
            "content": f"""Problem Statement:\n{problem_statement}\n\nInitial python files:\n{code_skeleton}\n\nTest Cases:\n{test_cases}\n\nGenerate the complete and correct implementation in python files that will pass all the provided test cases.\n\nSTRICT REQUIREMENT: You **MUST** output the **file name** along with file content.\n\nMUST IMPLEMENT ALL REQUIRED VARIABLES, CLASSES AND FUNCTIONS FROM STATEMENT AND ENSURE THEY PASS ALL TEST CASES.\n\nThe solution should be designed to pass all the test cases provided above.\nexample:\n```python\na.py\ncontents of a.py\n\nb.py\ncontents of b.py\n```"""
        }
    ]
    ...
    final_check_messages = [
        ...
        {
            "role": "user",
            "content": f"Generated Code:\n{protocol_check_response}\n\nProblem Statement:\n{problem_statement}\n\nCode Skeleton:\n{code_skeleton}\n\nTest Cases:\n{test_cases}\n\nPerform final validation: check algorithm correctness, semantic alignment, edge cases, and specification compliance. Ensure the solution will pass all test cases. Provide a corrected version if any issues are found. Return ONLY the final Python code with file names."
        }
    ]
    ...
    logger.info("[VALIDATION COMPLETE] 5-stage validation finished with test-driven approach")
    return solution
...
def process_fix_task(input_dict: Dict[str, Any]):
    ...
    try:
        ...
        patch_text= fix_task_solve_workflow_fix(
            problem_text,
            temperature=0.0,
            timeout=timeout,
            run_id_1=run_id,
...
def fix_task_solve_workflow_fix(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "", \
    test_runner: str = "pytest", test_runner_mode: str = "FILE", temperature: float, n_max_steps = MAX_FIX_TASK_STEPS) -> tuple[str, List[str], List[str]]:
    global run_id
    run_id=run_id_1
    ...
    for step in range(n_max_steps):
        ...
        try:
            next_thought, next_tool_name, next_tool_args,raw_text,total_attempts,error_counter,messages = EnhancedNetwork.inference(messages, model=GLM_MODEL_NAME, run_id=run_id, temperature=temperature)
        except Exception as e:
            import traceback  # Ensure traceback is accessible
        ...
        try:
            ...
            # Check if the observation contains an error message
            is_error = next_observation.startswith("Error in") or next_observation.startswith("Error:")
            cot.add_action(EnhancedCOT.Action(next_thought=next_thought,next_tool_name=next_tool_name,next_tool_args=next_tool_args,observation=next_observation,is_error=is_error,raw_response=raw_text,total_attempts=total_attempts,inference_error_counter=error_counter,request_data=messages))
        except Exception as e:
            import traceback  # Ensure traceback is accessible
            error_msg=f"observation: Unexpected error in tool execution: {str(e)}"
...
def create_task_solve_workflow_fix(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "", \
    test_runner: str = "pytest", test_runner_mode: str = "FILE", temperature: float, test_files: List[str] = None, n_max_steps = MAX_FIX_TASK_STEPS) -> tuple[str, List[str], List[str]]:
    """
    Create task workflow that implements solutions based on problem statement and test files.
    
    Args:
        problem_statement: The problem description to solve
        timeout: Maximum execution time in seconds
        run_id_1: Unique identifier for this run
        instance_id: Instance identifier (optional)
        test_runner: Test runner command (default: pytest)
        test_runner_mode: Test runner mode - FILE or MODULE (default: FILE)
        temperature: LLM temperature for generation
        test_files: List of test file paths to validate against (optional)
        n_max_steps: Maximum number of execution steps (default: MAX_FIX_TASK_STEPS)
    
    Returns:
        Tuple containing patch text, logs, and other metadata
    """
    global run_id
    run_id=run_id_1
    cot=EnhancedCOT(latest_observations_to_keep=30)
    ...
    logger.info(f"Starting main agent execution...")
    system_prompt = CREATE_TASK_SYSTEM_PROMPT.format(tools_docs=tool_manager.get_tool_docs(),format_prompt=FORMAT_PROMPT_V0)
    
    # Prepare test files information
    test_files_info = ""
    if test_files:
        test_files_info = "\n".join([f"- {test_file}" for test_file in test_files])
        test_files_info = f"The following test files are provided for validation:\n{test_files_info}\n\nUse these test files to guide your implementation and validate your solution."
    else:
        test_files_info = "No specific test files provided. You may need to create your own validation approach."
    
    instance_prompt = CREATE_TASK_INSTANCE_PROMPT_TEMPLATE.format(
