...
class BugFixSolver(BaseSolver):
    ...
    <system_prompt>
        ...
        <guidelines>
            <guideline>You are working in a codebase with other engineers and many different components. Be careful that changes you make in one component don't break other components.</guideline>
            <guideline>When designing changes, implement them as a senior software engineer would. This means following best practices such as separating concerns and avoiding leaky interfaces.</guideline>
            <guideline>When possible, choose the simpler solution.</guideline>
            <guideline>Use your bash tool to set up any necessary environment variables, such as those needed to run tests.</guideline>
            <guideline>You should run relevant tests to verify that your changes work.</guideline>
            ...
            <guideline>If you get the same error twice, STOP and try a completely different approach.</guideline>
            <guideline>Always check file existence before running files: use 'ls -la' or 'find . -name "*.py"' first.</guideline>
            <guideline>If you need a file that doesn't exist, CREATE it with str_replace_editor instead of trying to run it.</guideline>
            <guideline>Use sequential_thinking to plan your approach and avoid repeating failed actions.</guideline>
        </guidelines>
        <validation_rules>
            **Pass-to-Pass (P2P) Validation**
                What to do:
            ...
            **Existing Functionality Validation**
                ...
                **Step 11: Final Validation & Completion**
                ...
                If ALL validation passes: Call the 'complete' tool
                If ANY test fails: Go back to the relevant step and iterate
        </validation_rules>
        ...
        <reminder>Remember to use the complete tool when you're finished with the task.</reminder>
    </system_prompt>
    ...
    <instruction_prompt>
            ...
            <resolution_steps>
                <step number="1">As a first step, it would be a good idea to explore the repo to familiarize yourself with its structure.</step>
                <step number="2">Create a script to reproduce the error and execute it with `python <filename.py>` using the BashTool, to confirm the error</step>
                ...
                <step number="4">Edit the sourcecode of the repo to resolve the issue</step>
                <step number="5">Rerun your reproduce script and confirm that the error is fixed!</step>
                <step number="6">Think about edgecases and make sure your fix handles them as well</step>
                <step number="7">Run select tests from the repo to make sure that your fix doesn't break anything else.</step>
            </resolution_steps>
...
class EnhancedNetwork:
    ...
    @classmethod
    def make_request(cls, messages: list[dict], model: Types.Model = KIMI_MODEL_NAME, temperature: float=1.0, attempt: int=5) -> str:
