...
MODEL_ROUTES = {
    ...
    "test_generation": QWEN_MODEL_NAME,  # Use strong model for initial test generation
    "test_repair": QWEN_MODEL_NAME,      # Use strong model for repairs
    "issue_morph": QWEN_MODEL_NAME,      # Can use lighter model for rephrasing
    "validation": QWEN_MODEL_NAME,       # Strong model for validation
}
...
CRITICAL REQUIREMENTS FOR TEST CASE GENERATION:

1. **Prioritize Problem Statement Cases**: 
   - If there are similar test cases or cases that catch the same scenario, prioritize the cases explicitly mentioned in the problem statement
   - Give highest priority to requirements directly stated in the problem
   - Remove duplicate or redundant test cases that test the same functionality

2. **Test Case Clarity and Uniqueness**:
   - Each test case must be clear and test a unique aspect of the functionality
   - If two test cases verify the same behavior, keep the one that is most explicitly mentioned in the problem statement
   - Avoid redundant tests that don't add value

3. **Comprehensive Edge Case Coverage - CHECKPOINT BASED**:
   For EACH variable, function, and class in the problem, systematically check the following categories:

   **CHECKPOINT A: Variable Edge Cases**
   □ None/null values
   □ Empty values (empty string, empty list, empty dict)
   □ Zero and negative numbers
   □ Maximum/minimum values (integer limits, string length limits)
   □ Special characters in strings (Unicode, escape sequences)
   □ Type mismatches (wrong data types)

   **CHECKPOINT B: Function Edge Cases**
   □ Functions with no arguments
   □ Functions with maximum number of arguments
   □ Functions with optional/default parameters
   □ Functions with variable-length arguments (*args, **kwargs)
   □ Recursive functions (base cases, infinite recursion risk)
   □ Functions with side effects (mutating global state)
   □ Pure functions (no side effects)
   □ Functions returning different types (None, primitive, object)
   □ Exception-raising functions
   □ Coroutine/generator functions

   **CHECKPOINT C: Class/Instance Edge Cases**
   ...
   □ Instance lifecycle (creation, modification, deletion)

   **CHECKPOINT D: Software Weaknesses & Bugs**
   □ Off-by-one errors (boundary conditions)
   □ Index out of bounds errors
   □ Division by zero
   □ Integer overflow/underflow
   □ Floating point precision issues
   □ Non-deterministic behavior (ensure reproducible tests by controlling variable factors)
   □ Collision and uniqueness issues (when multiple instances need distinct identities)
   □ State persistence across test runs (ensure clean state for each test)
   □ Race conditions (if applicable)
   □ Memory leaks (large data structures)
   □ Infinite loops or recursion
   □ Deadlock or livelock conditions
   □ State inconsistency
   □ Resource cleanup issues (file handles, connections)
   □ Encoding/decoding errors
   □ Concurrent modification errors
   □ Pool exhaustion scenarios (resource pools that can run out)
   □ Lazy vs eager initialization conflicts

   **CHECKPOINT E: Integration & Interaction Edge Cases**
   □ Multiple functions calling each other
   □ Circular dependencies
   □ Callback functions with edge case inputs
   □ Event handlers with unexpected data
   □ Data flow through multiple transformations
   □ State changes over time (history-dependent behavior)

4. **Generation Limits**:
   - You have generation limit of 2048 tokens. Stop generating when near the limit
   - If you get syntax error, check if last assistant response was truncated. If yes, skip last couple of test cases to fit in
   - Prioritize test cases based on importance: problem statement cases first, then edge cases, then general cases

5. **Test Case Ordering Priority**:
   1. Test cases explicitly mentioned in problem statement (HIGHEST PRIORITY)
   2. Test cases related to core functionality requirements
   3. Edge case tests covering CHECKPOINTS A-E (boundary conditions, empty inputs, etc.)
   4. Deterministic behavior tests (ensure tests produce consistent results across multiple runs)
   5. Uniqueness and collision tests (verify distinct identities when required)
   6. State management tests (clean initialization, no cross-test contamination)
   7. Error handling and exception tests
   8. Integration and interaction tests
   9. General validation tests (LOWEST PRIORITY)

6. **Explicit Edge Case Identification**:
   - For each test case, identify which checkpoint it covers
   - Add comments to test cases indicating the specific edge case being tested
   - Example: `# CHECKPOINT A: Testing None value handling in process_data()`
   - Example: `# CHECKPOINT B: Testing function with invalid argument types`
   - Example: `# CHECKPOINT D: Testing deterministic behavior and uniqueness enforcement`
   - Example: `# CHECKPOINT D: Testing state isolation between multiple invocations`
   
7. **Critical Testing Principles for Non-Deterministic Code**:
   - When testing functions that involve random generation or dynamic behavior, ensure test determinism:
     * Control the variability factors that affect output (use mechanisms that ensure reproducible runs)
     * Test that each invocation produces unique results when uniqueness is required
     * Verify that consecutive calls don't produce identical outputs when they shouldn't
     * Ensure test isolation so one test's state doesn't affect another
   - When testing functions that manage shared resources or pools:
     * Test resource exhaustion scenarios
     * Verify proper resource allocation and deallocation
     * Test conflicts when resources are limited
   - When testing functions with internal state:
     * Verify clean state initialization on each call
     * Test state persistence when needed and state reset when required
     * Ensure no cross-contamination between test runs
...
class TestFuncA(unittest.TestCase):
    ...
    # CHECKPOINT: Core functionality - problem statement requirement
    def test_main_func_basic(self):
        self.assertEqual(main_func(), "expected_output")
    
    # CHECKPOINT B: Testing function with None input
    def test_main_func_with_none(self):
        self.assertIsNone(main_func(None))
    
    # CHECKPOINT B: Testing function with empty string input
    def test_main_func_with_empty_string(self):
        self.assertEqual(main_func(""), "")
    
    # CHECKPOINT D: Testing deterministic behavior across multiple calls
    def test_main_func_uniqueness_when_required(self):
        # Verify consecutive calls produce unique results when uniqueness is a requirement
        result1 = main_func()
        result2 = main_func()
        result3 = main_func()
        # If the function should produce unique outputs, ensure they are distinct
...
"""
You are an expert Python developer. Your task is to generate a complete, working Python solution for the given problem statement and test cases.

CRITICAL APPROACH: Test-Driven and Requirement-First Reasoning

1. **Extract Requirements First** (DO THIS BEFORE CODING):
   - Analyze the test cases to extract ALL explicit requirements, constraints, and expected behaviors
   - Identify implicit requirements from test structure and assertions
   - Document your understanding of what constitutes a "correct" solution
   - DO NOT rely on remembered examples or hard-coded patterns
   - Derive implementation strategy from the problem's stated requirements

2. **Test-Driven Design**:
   - For each test case, understand what behavior it expects
   - Map each behavior to implementation requirements
   - Design your solution to satisfy the observed test patterns
   - Ensure your implementation aligns with test expectations, not assumption-based hard-coding

3. **Robust Design Principles Over Quick Fixes**:
   - Prioritize correctness under ALL edge cases (e.g., name collisions, deterministic testing, resource exhaustion)
   - Choose scalable design patterns (lazy initialization, pool-based strategies, factory patterns) over single-solution tricks
   - Make behavior verifiable and testable
   - Avoid brittle assumptions (e.g., fixed seeds, expected sequences, hard-coded values)
   - Design for determinism in test environments (state reset between tests, collision-avoidance for random generation)

4. **Verify Assumptions Systematically**:
   - Outline expected behaviors BEFORE implementing
   - For each code decision, specify which test cases it affects and why
   - Map every implementation choice to observable test outcomes
   - If a design requires state management, document clear reset semantics
   - Test both immediate and lazy behaviors explicitly

5. **Handle Common Evaluation Pitfalls**:
   - Detect and resolve conflicts between random generation and test determinism
   - Implement collision-avoidance strategies compatible with repeated test runs
   - Ensure test isolation (no shared state across tests)
   - Handle boundary conditions explicitly (empty inputs, maximum values, overflow conditions)
...
class EnhancedNetwork:
    ...
    def make_request(cls,messages:list,model:str,attempt:int=0, temperature:float=0.0, top_p:float=1, frequency_penalty:float=0.0, presence_penalty:float=0.0, max_retries:int=5)->str:
        ...
        logger.info("[REQUEST] run_id:", run_id)
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def __init__(self, available_tools: Optional[list[str]] = [], test_runner: str = "pytest", test_runner_mode: str = "FILE"):
        ...
        # Re-add test quality tracking from old version
        self.test_results_history = []
        self.baseline_test_results = None
        self.current_test_results = None
        self.affected_test_files = []
        self.fix_quality_score = 0
    ...
    def parse_test_results(self, test_output: str) -> dict:
        """Parse test output to extract pass/fail statistics and error details."""
        result = {
            'passed': 0,
            'failed': 0,
            'errors': 0,
        ...
        if 'pytest' in self.test_runner.lower() or '=== test session starts ===' in test_output.lower():
            summary_patterns = [
                r'(\d+)\s+passed',
                r'(\d+)\s+failed',
                r'(\d+)\s+error',
            ...
            for pattern in summary_patterns:
                ...
                if matches:
                    ...
                    if 'passed' in pattern:
                        ...
                        result['errors'] = count
            failure_blocks = re.findall(r'FAILED (.*?) -', test_output)
            result['failure_details'] = failure_blocks[:10] 
            error_blocks = re.findall(r'(AssertionError|Error|Exception):.*', test_output)
            result['error_messages'] = error_blocks[:10]  
        elif 'unittest' in self.test_runner.lower() or 'Ran ' in test_output:
            ran_match = re.search(r'Ran\s+(\d+)\s+test', test_output)
            if ran_match:
                result['total'] = int(ran_match.group(1))
            if re.search(r'\bOK\b', test_output):
                result['passed'] = result.get('total', 0)
            else:
    ...
    def calculate_quality_score(self, baseline: dict, current: dict) -> tuple[int, str]:
        """Calculate quality improvement score comparing baseline vs current test results."""
        if not baseline or not current:
            return 0, "Insufficient test data for quality assessment"
        passed_improvement = current['passed'] - baseline['passed']
        failed_improvement = baseline['failed'] - current['failed']
        error_improvement = baseline['errors'] - current['errors']
        score = (passed_improvement * 10) + (failed_improvement * 15) + (error_improvement * 15)
        score = max(-100, min(100, score))
        if score >= 50:
            feedback = f"✅ Excellent improvement! Passed tests increased by {passed_improvement}, failed tests decreased by {-failed_improvement}."
        elif score >= 20:
            feedback = f"✓ Good progress. Passed: {current['passed']}/{current['total']}, improvement: {passed_improvement} tests."
        elif score >= 0:
            feedback = f"→ Slight improvement. Passed: {current['passed']}/{current['total']}. Consider refining the solution."
        elif score >= -20:
            feedback = f"⚠ Minor regression detected. {-passed_improvement} tests now failing. Review the changes."
        else:
            feedback = f"❌ Significant regression! {-failed_improvement} more tests failing. Revert or reconsider the approach."
        return score, feedback

    def identify_affected_tests(self, file_path: str) -> list[str]:
        """Identify test files that might be affected by changes to the given file."""
        affected = []
                ...
                if file.startswith('test_') and file.endswith('.py'):
                    ...
                    try:
                        with open(test_path, 'r', encoding='utf-8') as f:
                            content = f.read()
                        if file_name in content or module_name in content or file_path in content:
                            affected.append(test_path)
                    except Exception as e:
                        logger.debug(f"Could not read test file {test_path}: {e}")
        return affected[:10]
    
    # ==================== e-Otter++ Execution Feedback Methods ====================
    ...
    def _detect_test_framework(self) -> str:
        ...
        # Check existing test files for framework indicators
        for root, _, files in os.walk('.'):
            for file in files:
                if file.startswith('test_') and file.endswith('.py'):
                    try:
        ...
        # Default to pytest (more modern)
        return 'pytest'
    
    def _extract_project_imports(self, target_file: str) -> list[str]:
        """Extract common imports from target file and related files"""
        imports = set()
        ...
        try:
            ...
            with open(target_file, 'r') as f:
                ...
                imports.update(import_lines[:10])  # Top 10 imports
        except:
            pass
        
        # Get imports from related test files
        target_module = os.path.basename(target_file).replace('.py', '')
        for root, _, files in os.walk('.'):
            for file in files:
                if f'test_{target_module}' in file:
                    try:
                        with open(os.path.join(root, file), 'r') as f:
                            content = f.read()
                            import_lines = re.findall(r'^(?:from|import)\s+[\w.]+.*$', content, re.MULTILINE)
                            imports.update(import_lines[:5])
                    except:
                        continue
        
        return list(imports)[:15]  # Return top 15
    
    def _find_similar_tests(self, target_file: str) -> list[str]:
        """Find similar test patterns from existing tests"""
        target_module = os.path.basename(target_file).replace('.py', '')
        test_examples = []
            ...
            for file in files:
                if file.startswith('test_') and file.endswith('.py'):
                    try:
                        ...
                        if test_funcs and len(test_examples) < 2:
                            # Get first complete test function
                            match = re.search(
                                r'(def test_\w+\(.*?\):(?:\n.*?){1,20})',
                                content,
                                re.DOTALL
                            ...
                            if match:
                                test_examples.append(match.group(1).strip())
                    except:
                        continue
        
        return test_examples[:2]  # Return top 2 examples
    
    def _morph_issue_description_v2(self, issue_desc: str) -> list[str]:
        """Generate multiple morphed versions of issue description"""
        morph_prompt = f"""Generate 3 different rephrasings of this bug report while preserving all technical details:
...
2. Emphasize different aspects (symptom vs cause vs impact)
3. Keep all technical details (function names, error messages, etc.)
...
"""
        ...
        try:
            response = EnhancedNetwork.make_request([
                {"role": "system", "content": "You are an expert at rephrasing technical documentation."},
                {"role": "user", "content": morph_prompt}
            ], model=MODEL_ROUTES["issue_morph"], temperature=0.7)
            
            # Parse variants
            variants = []
            for i in range(1, 4):
                pattern = f"VARIANT_{i}:(.*?)(?=VARIANT_|$)"
                match = re.search(pattern, response, re.DOTALL)
                if match:
                    variants.append(match.group(1).strip())
            
            return variants if variants else [issue_desc]
        except:
            return [issue_desc]
    
    def _extract_bug_keywords(self, issue_desc: str) -> list[str]:
        """Extract key technical terms from issue description"""
        # Extract function names, class names, error types
        keywords = []
        
        # Function/method names
        func_matches = re.findall(r'\b[a-z_]+\([^)]*\)', issue_desc)
        keywords.extend([m.split('(')[0] for m in func_matches])
        
        # Class names (capitalized words)
        class_matches = re.findall(r'\b[A-Z][a-zA-Z]+\b', issue_desc)
        keywords.extend(class_matches)
        
        # Error types
        error_matches = re.findall(r'\b\w+Error\b|\b\w+Exception\b', issue_desc)
        keywords.extend(error_matches)
        
        return list(set(keywords))[:10]
    
    def _get_heterogeneous_prompts(self, issue_description: str, target_file: str) -> list[dict]:
        """Generate diverse prompts with codebase context and temperature variation"""
        
        # Extract codebase context
        framework = self._detect_test_framework()
        imports = self._extract_project_imports(target_file)
        test_examples = self._find_similar_tests(target_file)
        keywords = self._extract_bug_keywords(issue_description)
        
        # Build context section
        context_block = ""
        if imports:
            context_block += f"\nCommon imports in this project:\n" + "\n".join(imports[:10])
        
        if test_examples:
            context_block += f"\n\nExample test patterns from this codebase:\n{test_examples[0][:300]}..."
        ...
        try:
            with open(target_file, 'r') as f:
                ...
                file_context = "".join(lines[:50])  # First 50 lines for context
        except:
            pass
        ...
        base_requirements = f"""
Requirements:
- Test MUST fail on buggy code and pass after fix
- Use {framework} framework
- Focus on bug-triggering inputs
- Include clear assertions
- Name test as test_reproduce_issue_[description]
"""
        
        prompts = []
        
        # ===== VARIANT 1: Exact Reproduction (Temperature 0.0) =====
        prompts.append({
            "variant": "exact_reproduction",
            "temperature": 0.0,
            "prompt": f"""Generate a precise test that reproduces this bug exactly as described.
...
FOCUS: Reproduce the exact error/behavior. Use specific inputs mentioned in the bug report.
"""
        })
        
        # ===== VARIANT 2: Edge Cases Focus (Temperature 0.3) =====
        prompts.append({
            "variant": "edge_cases",
            "temperature": 0.3,
            "prompt": f"""Generate a test that explores boundary conditions related to this bug.
...
FOCUS: Test edge cases (empty input, None, extremes, special chars). Think about what boundary conditions trigger this bug.
"""
        })
        
        # ===== VARIANT 3: Minimal Reproduction (Temperature 0.0) =====
        prompts.append({
            "variant": "minimal",
            "temperature": 0.0,
            "prompt": f"""Generate the SIMPLEST possible test that reproduces this bug.
...
FOCUS: Absolute minimum code. Single assertion. One clear test case. No extra setup.
"""
        })
        
        # ===== VARIANT 4: Context-Aware (Temperature 0.3) =====
        prompts.append({
            "variant": "context_aware",
            "temperature": 0.3,
            "prompt": f"""Generate a test using this project's patterns and conventions.
...
FOCUS: Follow existing project test patterns. Use same imports, style, and conventions.
"""
        })
        
        # ===== VARIANT 5: Multiple Assertions (Temperature 0.5) =====
        prompts.append({
            "variant": "multi_assertion",
            "temperature": 0.5,
            "prompt": f"""Generate a comprehensive test with multiple assertions to verify this bug.
...
FOCUS: Multiple related assertions. Test pre-conditions, the bug itself, and side effects.
"""
        })
        
        # ===== VARIANT 6: Data-Driven (Temperature 0.3) =====
        prompts.append({
            "variant": "data_driven",
            "temperature": 0.3,
            "prompt": f"""Generate a parametrized test with multiple test cases for this bug.
...
FOCUS: Use pytest.mark.parametrize (or unittest subTest). Test multiple input combinations that should trigger the bug.
"""
        })
        
        # ===== VARIANT 7: Error Message Focus (Temperature 0.0) =====
        prompts.append({
            "variant": "error_message",
            "temperature": 0.0,
            "prompt": f"""Generate a test that verifies the specific error/exception mentioned.
...
FOCUS: If an exception is mentioned, use pytest.raises or assertRaises. Verify exact error type and message.
"""
        })
        
        # ===== VARIANT 8: Context-Masked (Temperature 0.7) =====
        masked_issue = self._apply_context_mask(issue_description)
        prompts.append({
            "variant": "context_masked",
            "temperature": 0.7,
            "prompt": f"""Generate a test based on this partial bug description. Infer missing details.

BUG REPORT (some details hidden):
{masked_issue}

TARGET FILE: {target_file}
{base_requirements}

FOCUS: Infer what's hidden. Think creatively about what could cause this issue.
"""
        })
        
        # ===== VARIANT 9: Setup/Teardown Focus (Temperature 0.3) =====
        prompts.append({
            "variant": "setup_teardown",
            "temperature": 0.3,
            "prompt": f"""Generate a test with proper setup and teardown for this bug.
...
FOCUS: Include setUp/tearDown or pytest fixtures if needed. Consider state, files, mocks, etc.
"""
        })
        
        # ===== VARIANT 10: Integration Style (Temperature 0.5) =====
        prompts.append({
            "variant": "integration",
            "temperature": 0.5,
            "prompt": f"""Generate an integration-style test that shows how this bug manifests in realistic usage.
...
FOCUS: Realistic workflow. Multiple function calls. Show the bug in a real-world scenario.
"""
        })
        
        # ===== VARIANT 11: Regression Focus (Temperature 0.0) =====
        prompts.append({
            "variant": "regression",
            "temperature": 0.0,
            "prompt": f"""Generate a regression test that will prevent this bug from returning.
...
FOCUS: Clear test name. Document WHY this test exists. Assert expected behavior explicitly.
"""
        })
        
        # ===== VARIANT 12: Alternative Approach (Temperature 0.7) =====
        prompts.append({
            "variant": "creative",
            "temperature": 0.7,
            "prompt": f"""Think creatively: generate a test that approaches this bug from a different angle.
...
FOCUS: Think outside the box. Different inputs, different call patterns, indirect triggering.
"""
        })
        
        return prompts
    
    def _apply_context_mask(self, issue_desc: str) -> str:
        """Apply context masking to force different perspectives (e-Otter++ technique)"""
        # Remove specific error messages to force inference
        masked = re.sub(r'Error: .*', 'Error: [details hidden]', issue_desc)
        masked = re.sub(r'Traceback.*', '[Traceback hidden]', masked, flags=re.DOTALL)
        return masked + "\n\n[Note: Some details masked - infer from description]"
    
    def _execute_test_with_categorization(self, test_code: str, test_file_path: str) -> dict:
        """Execute test and categorize the result for repair decisions"""
        ...
        try:
            ...
            # Execute test
            result = self.run_repo_tests([test_file_path])
            parsed = self.parse_test_results(result)
            
            # Categorize result
            if parsed['failed'] > 0 and parsed['errors'] == 0:
                # Test fails with assertion - this is what we want!
                return {
                    'type': 'valid_failure',
                    'result': parsed,
                    'message': 'Test correctly fails with assertion error',
                    'raw_output': result
                }
            elif parsed['errors'] > 0:
                # Test has execution errors (syntax, import, etc.)
                error_msgs = parsed.get('error_messages', [])
                return {
                    'type': 'invalid_failure',
                    'result': parsed,
                    'error': '\n'.join(error_msgs[:3]),
                    'message': 'Test has execution errors',
                    'raw_output': result
                }
            elif parsed['passed'] > 0:
                # Test passes - doesn't reproduce the issue!
                return {
                    'type': 'false_pass',
                    'result': parsed,
                    'message': 'Test passes but should fail to reproduce issue',
                    'raw_output': result
                }
            else:
                return {
                    'type': 'unknown',
                    'result': parsed,
                    'message': 'Unexpected test result',
                    'raw_output': result
                }
            ...
            return {
                ...
                'error': 'Test took too long to execute'
            }
        except Exception as e:
            return {
                'type': 'invalid_failure',
    ...
    def _create_repair_prompt(self, test_code: str, execution_result: dict, issue_description: str) -> str:
        """Create repair prompt based on execution feedback (e-Otter++ core technique)"""
        
        result_type = execution_result['type']
        
        if result_type == 'invalid_failure':
            return f"""The test has execution errors. Please fix it.
...
Instructions:
1. Fix the syntax, import, or runtime errors
2. Ensure the test executes successfully
3. Make sure it still reproduces the bug from the issue
4. Return ONLY the corrected test code without explanations

Corrected Test Code:"""

        elif result_type == 'false_pass':
            return f"""The test PASSES on buggy code, but it should FAIL to reproduce the issue.
...
Problem: The test doesn't actually trigger the bug described in the issue.

Instructions:
1. Revise the test to actually trigger the bug
2. The test should FAIL on the current buggy code
3. The test should PASS once the bug is fixed
4. Return ONLY the revised test code without explanations

Revised Test Code:"""

        elif result_type == 'timeout':
            return f"""The test timed out during execution.
...
Instructions:
1. Remove any infinite loops or long-running operations
2. Keep the test focused and quick
3. Still reproduce the bug from the issue
4. Return ONLY the revised test code without explanations

Revised Test Code:"""
        
        else:
            return f"""Please improve this test to better reproduce the issue.
...
Return ONLY the improved test code:"""
    ...
    def _generate_test_from_prompt(self, prompt: str, model: str = None, temperature: float = 0.0, task_type: str = "test_generation") -> str:
        """Generate test code from prompt using LLM with intelligent model routing"""
        if model is None:
            model = MODEL_ROUTES.get(task_type, MODEL_ROUTES["default"])
        
        messages = [
            {"role": "system", "content": "You are an expert test developer. Generate only the test code without explanations."},
            {"role": "user", "content": prompt}
        ]
        
        response = EnhancedNetwork.make_request(messages, model=model, temperature=temperature)
    ...
    def _calculate_test_quality(self, test_code: str, execution_result: dict) -> float:
        """Calculate sophisticated quality score for a test based on multiple metrics"""
        score = 0.0
        
        # ===== CORE REQUIREMENT: Valid Failure =====
        if execution_result['type'] == 'valid_failure':
            score += 100.0
        else:
            # If not valid failure, it's not a good test
            return score
        
        # ===== METRIC 1: Code Brevity (shorter is better) =====
        lines = len([l for l in test_code.split('\n') if l.strip()])
        if lines <= 10:
            score += 15.0  # Very concise
        elif lines <= 20:
            score += 10.0  # Good size
        elif lines <= 35:
            score += 5.0   # Acceptable
        elif lines > 50:
            score -= 5.0   # Too long
        
        # ===== METRIC 2: Assertion Quality =====
        assert_count = test_code.lower().count('assert')
        if assert_count == 1:
            score += 10.0  # Single focused assertion (ideal)
        elif assert_count == 2:
            score += 8.0   # Two assertions (good)
        elif assert_count >= 3:
            score += 5.0   # Multiple assertions (acceptable)
        
        # Reward specific assertions
        if 'assertRaises' in test_code or 'pytest.raises' in test_code:
            score += 5.0  # Exception testing
        if 'assertEqual' in test_code or '==' in test_code:
            score += 3.0  # Specific equality
        
        # ===== METRIC 3: Test Structure =====
        # Good test name
        if re.search(r'def test_\w+', test_code):
            score += 5.0
        if 'reproduce' in test_code.lower() or 'bug' in test_code.lower():
            score += 3.0  # Clear naming
        
        # Has docstring explaining the test
        if '"""' in test_code or "'''" in test_code:
            score += 5.0
        
        # ===== METRIC 4: Minimal Dependencies =====
        import_count = len(re.findall(r'^\s*(?:from|import)\s+', test_code, re.MULTILINE))
        if import_count <= 3:
            score += 5.0  # Minimal imports
        elif import_count <= 5:
            score += 3.0  # Acceptable imports
        
        # ===== METRIC 5: Specificity =====
        # Test uses specific values/constants (not just variables)
        literals = len(re.findall(r'\b\d+\b|["\'][\w\s]+["\']', test_code))
        if literals >= 2:
            score += 5.0  # Concrete test data
        
        # ===== METRIC 6: Execution Quality =====
        # Clear failure message
        result_data = execution_result.get('result', {})
        if result_data.get('failed', 0) > 0:
            score += 5.0  # Properly fails
        
        # Fast execution (no timeouts)
        if execution_result.get('type') != 'timeout':
            score += 5.0
        
        # ===== METRIC 7: Complexity Penalty =====
        # Penalize overly complex tests
        if 'while' in test_code or 'for' in test_code:
            score -= 2.0  # Loops make tests less clear
        if test_code.count('if ') > 2:
            score -= 2.0  # Too many conditionals
        if 'mock' in test_code.lower() or 'patch' in test_code.lower():
            score -= 3.0  # Mocking adds complexity
        
        # ===== METRIC 8: Maintainability =====
        # No hardcoded paths
        if '/tmp/' in test_code or 'C:\\' in test_code:
            score -= 5.0
        
        # Uses parameterization (advanced technique)
        if '@pytest.mark.parametrize' in test_code or 'subTest' in test_code:
            score += 10.0  # Excellent practice
    ...
    @EnhancedToolManager.tool
    def generate_and_repair_test(
        self,
        issue_description: str,
        target_file: str,
        max_repair_attempts: int = 3,
        use_heterogeneous_prompts: bool = True
    ) -> str:
        '''
        Generates a reproduction test with execution-feedback repair loop (e-Otter++ technique).
        Ensures test fails on current (buggy) code through iterative repair based on execution feedback.
        
        Arguments:
            issue_description: Description of the bug to reproduce
            target_file: File containing the buggy code
            max_repair_attempts: Maximum repair iterations per prompt variant (default: 3)
            use_heterogeneous_prompts: Use diverse prompts for better coverage (default: True)
            
        Output:
            Generated test code that successfully reproduces the issue with fail-to-pass behavior
        '''
        
        logger.info("[e-Otter++] ========== STARTING EXECUTION-FEEDBACK TEST GENERATION ==========")
        logger.info(f"[e-Otter++] Target file: {target_file}")
        logger.info(f"[e-Otter++] Max repair attempts: {max_repair_attempts}")
        logger.info(f"[e-Otter++] Heterogeneous prompts: {use_heterogeneous_prompts}")
        
        # Generate issue morphs (rephrased versions)
        issue_morphs = [issue_description]  # Original
        if use_heterogeneous_prompts:
            try:
                morphed = self._morph_issue_description_v2(issue_description)
                issue_morphs.extend(morphed)
                logger.info(f"[e-Otter++] Generated {len(morphed)} issue description morphs")
            except Exception as e:
                logger.warning(f"[e-Otter++] Failed to generate issue morphs: {e}")
        
        # Get prompts (heterogeneous or single)
        prompts = self._get_heterogeneous_prompts(issue_description, target_file) if use_heterogeneous_prompts else [
            {"variant": "default", "prompt": f"Generate test for: {issue_description}", "temperature": 0.0}
        ]
        
        best_test = None
        best_score = -1
        test_file_path = f"test_reproduction_{uuid4().hex[:8]}.py"
        
        try:
            # Try each issue morph with subset of prompts
            for morph_idx, current_issue_desc in enumerate(issue_morphs):
                logger.info(f"[e-Otter++] === Using issue description variant {morph_idx + 1}/{len(issue_morphs)} ===")
                
                # Select prompts for this morph (rotate through prompts)
                prompts_to_try = prompts if morph_idx == 0 else prompts[morph_idx*3:(morph_idx+1)*3]
                if not prompts_to_try:
                    prompts_to_try = prompts[:3]
                
                for prompt_idx, prompt_config in enumerate(prompts_to_try):
                    variant = prompt_config['variant']
                    prompt_template = prompt_config['prompt']
                    temperature = prompt_config.get('temperature', 0.0)
                    
                    # Replace issue description in prompt with current morph
                    prompt = prompt_template.replace(issue_description, current_issue_desc)
                    
                    logger.info(f"[e-Otter++] --- Variant: {variant}, Temp: {temperature}, Morph: {morph_idx} ---")
                    
                    # Generate initial test
                    try:
                        test_code = self._generate_test_from_prompt(prompt, temperature=temperature)
                        logger.info(f"[e-Otter++] Generated initial test ({len(test_code)} chars)")
                    except Exception as e:
                        logger.warning(f"[e-Otter++] Failed to generate test: {e}")
                        continue
                    
                    # Repair loop for this specific prompt variant
                    for attempt in range(max_repair_attempts):
                        logger.info(f"[e-Otter++] Repair attempt {attempt + 1}/{max_repair_attempts}")
                        
                        # Execute and categorize result
                        try:
                            result = self._execute_test_with_categorization(test_code, test_file_path)
                            logger.info(f"[e-Otter++] Execution result: {result['type']} - {result['message']}")
                        except Exception as e:
                            logger.warning(f"[e-Otter++] Execution failed: {e}")
                            break
                        
                        if result['type'] == 'valid_failure':
                            # Success! Test correctly fails on buggy code
                            logger.info(f"[e-Otter++] ✓✓✓ VALID FAILURE achieved! Test reproduces the issue.")
                            
                            score = self._calculate_test_quality(test_code, result)
                            logger.info(f"[e-Otter++] Test quality score: {score}")
                            
                            if score > best_score:
                                best_test = test_code
                                best_score = score
                                logger.info(f"[e-Otter++] New best test (score: {score})")
                            break
                        
                        elif result['type'] in ['invalid_failure', 'false_pass', 'timeout']:
                            # Need to repair
                            logger.warning(f"[e-Otter++] Test needs repair: {result['type']}")
                            
                            try:
                                repair_prompt = self._create_repair_prompt(test_code, result, current_issue_desc)
                                test_code = self._generate_test_from_prompt(repair_prompt, temperature=temperature, task_type="test_repair")
                                logger.info(f"[e-Otter++] Repaired test generated")
                            except Exception as e:
                                logger.warning(f"[e-Otter++] Repair failed: {e}")
                                break
                        else:
                            logger.warning(f"[e-Otter++] Unknown result type: {result['type']}")
                            break
            
            if best_test is None:
                raise EnhancedToolManager.Error(
                    EnhancedToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                    f"Failed to generate valid reproduction test after trying {len(prompts)} variants with {max_repair_attempts} repair attempts each"
                )
            
            logger.info(f"[e-Otter++] ========== SUCCESS! Best test score: {best_score} ==========")
            return best_test
            ...
            if os.path.exists(test_file_path):
                try:
                    ...
                    logger.debug(f"[e-Otter++] Cleaned up temp file: {test_file_path}")
                except Exception:
                    pass
    ...
    def validate_fail_to_pass(
        ...
        '''
        Validates that test exhibits fail-to-pass (F→P) behavior - core validation from e-Otter++.
        Tests should fail on buggy code and pass on fixed code.
        
        Arguments:
            test_code: The test code to validate
            test_file_path: Path where test should be saved
            target_code_file: Path to file containing the buggy code
            fixed_code_snippet: Optional: specific fixed code to test against (if None, assumes current code is fixed)
            
        Output:
            Validation result indicating F→P status with detailed diagnostics
        '''
        
        logger.info("[F→P] ========== VALIDATING FAIL-TO-PASS BEHAVIOR ==========")
        
        # Save test
        try:
            with open(test_file_path, 'w') as f:
                f.write(test_code)
            logger.info(f"[F→P] Test saved to: {test_file_path}")
        except Exception as e:
            raise EnhancedToolManager.Error(
                EnhancedToolManager.Error.ErrorType.FILE_NOT_FOUND.name,
                f"Failed to save test file: {e}"
            )
        
        try:
            # If fixed code provided, need to test both versions
            if fixed_code_snippet:
                # Backup original
                original_code = self._get_file_content(target_code_file, limit=-1)
                
                # Test with buggy code (current)
                logger.info("[F→P] Testing with BUGGY code...")
                buggy_result = self.run_repo_tests([test_file_path])
                buggy_parsed = self.parse_test_results(buggy_result)
                logger.info(f"[F→P] Buggy code results: {buggy_parsed}")
                
                try:
                    # Apply fix temporarily
                    logger.info("[F→P] Applying fix temporarily...")
                    with open(target_code_file, 'w') as f:
                        f.write(fixed_code_snippet)
                    
                    # Test with fixed code
                    logger.info("[F→P] Testing with FIXED code...")
                    fixed_result = self.run_repo_tests([test_file_path])
                    fixed_parsed = self.parse_test_results(fixed_result)
                    logger.info(f"[F→P] Fixed code results: {fixed_parsed}")
                finally:
                    # Always restore original, even if exception occurs
                    logger.info("[F→P] Restoring original code...")
                    try:
                        with open(target_code_file, 'w') as f:
                            f.write(original_code)
                    except Exception as restore_error:
                        logger.error(f"[F→P] CRITICAL: Failed to restore original code: {restore_error}")
                        raise
            else:
                # Just test current state (assume it's buggy)
                logger.info("[F→P] Testing current code state...")
                test_result = self.run_repo_tests([test_file_path])
                buggy_parsed = self.parse_test_results(test_result)
                fixed_parsed = None
                logger.info(f"[F→P] Current state results: {buggy_parsed}")
            
            # Analyze F→P behavior
            if fixed_parsed:
                # Full F→P validation
                fails_on_buggy = buggy_parsed['failed'] > 0 or buggy_parsed['errors'] > 0
                passes_on_fixed = fixed_parsed['passed'] > 0 and fixed_parsed['failed'] == 0 and fixed_parsed['errors'] == 0
                
                is_f2p = fails_on_buggy and passes_on_fixed
                
                if is_f2p:
                    return f"""✅✅✅ FAIL-TO-PASS VALIDATED ✅✅✅

Test exhibits proper F→P behavior:
- FAILS on buggy code: {buggy_parsed['failed']} failures, {buggy_parsed['errors']} errors
- PASSES on fixed code: {fixed_parsed['passed']} passed

This test correctly:
1. Reproduces the issue on buggy code
2. Validates the fix on corrected code
3. Can be used to prevent future regressions

F→P Status: ✓ VALIDATED"""
                else:
                    return f"""❌ FAIL-TO-PASS VALIDATION FAILED ❌

Test does not exhibit proper F→P behavior:
- Buggy code: {buggy_parsed['passed']} passed, {buggy_parsed['failed']} failed, {buggy_parsed['errors']} errors
- Fixed code: {fixed_parsed['passed']} passed, {fixed_parsed['failed']} failed, {fixed_parsed['errors']} errors

Issues:
- Fails on buggy: {fails_on_buggy} (should be True)
- Passes on fixed: {passes_on_fixed} (should be True)

F→P Status: ✗ NOT VALIDATED"""
            else:
                # Partial validation (just checking it fails)
                fails_currently = buggy_parsed['failed'] > 0 or buggy_parsed['errors'] > 0
                
                if fails_currently:
                    return f"""⚠️ PARTIAL VALIDATION (Buggy Code Only)

Test FAILS on current buggy code:
- Failures: {buggy_parsed['failed']}
- Errors: {buggy_parsed['errors']}
- Total: {buggy_parsed['total']}

✓ Test reproduces the issue
⚠️ Cannot verify it passes on fixed code (no fix provided)

To complete F→P validation, provide fixed_code_snippet parameter."""
                else:
                    return f"""❌ VALIDATION FAILED - Test Does Not Fail

Test PASSES on current code but should FAIL to reproduce issue:
- Passed: {buggy_parsed['passed']}
- Failed: {buggy_parsed['failed']}
- Errors: {buggy_parsed['errors']}

The test does not reproduce the issue described.
Consider using generate_and_repair_test for better results."""
            ...
            if os.path.exists(test_file_path):
                try:
                    os.remove(test_file_path)
                    logger.debug(f"[F→P] Cleaned up test file: {test_file_path}")
                except Exception:
                    pass
    
    @EnhancedToolManager.tool
    def analyze_test_failures(self, test_files: List[str] = None) -> str:
        '''
        Analyzes test failures to provide actionable insights and track quality improvements.
        This tool parses test output, compares against baseline, and provides detailed diagnostics.
        
        Arguments:
            test_files: Optional list of specific test files to analyze. If None, uses affected_test_files if available, otherwise analyzes all test files.
            
        Output:
            Detailed analysis including:
            - Test pass/fail statistics
            - Quality score compared to baseline
            - Specific failure details and error messages
            - Recommendations for fixes
        '''
        if test_files is None:
            # Prioritize affected tests from recent code edits
            if self.affected_test_files:
                test_files = self.affected_test_files
                logger.info(f"Using {len(test_files)} affected test files identified from recent edits")
            else:
                # Fall back to finding all test files
                test_files = []
                for root, _, files in os.walk('.'):
                    for file in files:
                        if file.startswith('test_') and file.endswith('.py'):
        ...
        if not test_files:
            return "No test files found for analysis."
        
        try:
            test_output = self.run_repo_tests(test_files)
            parsed_results = self.parse_test_results(test_output)
            
            self.current_test_results = parsed_results
            self.test_results_history.append(parsed_results)
            
            if self.baseline_test_results is None:
                self.baseline_test_results = parsed_results
                analysis = f"""
📊 **Baseline Test Results Established**
- Total Tests: {parsed_results['total']}
- Passed: {parsed_results['passed']}
- Failed: {parsed_results['failed']}
- Errors: {parsed_results['errors']}

This will be used as reference for measuring improvements.
"""
            else:
                score, feedback = self.calculate_quality_score(self.baseline_test_results, parsed_results)
                self.fix_quality_score = score
...
- Passed: ✅ {parsed_results['passed']}
- Failed: ❌ {parsed_results['failed']}
- Errors: ⚠️  {parsed_results['errors']}

**Baseline Comparison:**
- Baseline Passed: {self.baseline_test_results['passed']}
- Current Passed: {parsed_results['passed']}
- Change: {parsed_results['passed'] - self.baseline_test_results['passed']:+d}

**Quality Score:** {score}/100
{feedback}
"""
                
                if parsed_results['failure_details']:
                    analysis += f"\n**Failed Tests:**\n"
                    for i, failure in enumerate(parsed_results['failure_details'][:5], 1):
                        analysis += f"{i}. {failure}\n"
                
                if parsed_results['error_messages']:
                    analysis += f"\n**Error Messages:**\n"
                    for i, error in enumerate(parsed_results['error_messages'][:5], 1):
                        analysis += f"{i}. {error}\n"
                
                if score < 0:
                    analysis += "\n⚠️ **Recommendation:** Tests are regressing. Consider reverting recent changes or using start_over tool."
                elif parsed_results['failed'] > 0:
                    analysis += "\n💡 **Recommendation:** Focus on fixing the failing tests listed above."
                else:
                    analysis += "\n✅ **All tests passing!** Ready to finish."
            
            return analysis
            
        except Exception as e:
            logger.error(f"Error analyzing test failures: {e}")
            return f"Error during test analysis: {str(e)}"
...
def generate_initial_solution_with_test_context(problem_statement: str, code_skeleton: str, test_cases: str, test_requirements: str) -> str:
    """
    Generate initial solution with explicit test case context and requirements analysis.
    This ensures the solution is designed to pass the test cases from the start.
    """
    models = determine_model_order(problem_statement)
    
    # Enhanced solution generation prompt that emphasizes test-driven approach
    solution_prompt = f"""
    CRITICAL: You are given test cases and their requirements analysis.
    ...
    TASK: Generate a complete solution that:
    1. Passes ALL provided test cases
    2. Implements all functions/classes with exact names from code skeleton
    3. Handles all edge cases identified in the test cases
    4. Uses robust design patterns (not brittle assumptions)
    5. Is deterministic and testable
    
    IMPLEMENTATION STRATEGY:
    - For each test case, ensure your solution produces the expected output
    - Use collision-avoidance strategies for random generation
    - Design for test determinism (no shared state, clear reset semantics)
    - Choose patterns that scale (lazy init, factory patterns, pool-based strategies)
    - Map each design decision to specific test case behaviors
    
    Return only the final Python code with file names.
    """
    ...
    try:
        response = EnhancedNetwork.make_request(messages, model=models[0], temperature=0.0)
        ...
        logger.info("Generated solution with test-driven approach")
        return solution
        
    except Exception as e:
        logger.error(f"Error generating solution with test context: {e}")
        # Fallback to original method
        return generate_initial_solution(problem_statement, code_skeleton)
...
def generate_initial_solution(problem_statement: str, code_skeleton: str) -> str:
    ...
    5. Presence and quality of edge case comments - solutions should have clear comments identifying each edge case being handled
    6. Completeness of edge case coverage - verify that all critical edge cases from the problem statement are addressed and properly commented
    7. Missing edge case analysis - consider potential edge cases that might not be handled
    8. Missing edge case coverage - consider potential edge cases problematically and programmatically
...
def analyze_test_requirements_for_solution(test_code: str, problem_statement: str) -> str:
    """
    Analyze test cases to extract requirements and expected behaviors for solution generation.
    This helps the LLM understand what the solution needs to satisfy.
    """
    analysis_prompt = textwrap.dedent("""
    Analyze the provided test cases and extract explicit requirements for the solution.
    
    For each test case, identify:
    1. What behavior is being tested?
    2. What inputs are provided?
    3. What outputs are expected?
    4. What edge cases or constraints are implied?
    5. What design patterns or strategies would satisfy these tests?
    
    Synthesize all test cases to produce a comprehensive requirement specification.
    DO NOT provide implementation code, only requirements and expected behaviors.
    ...
    messages = [
        ...
        {"role": "user", "content": f"Problem Statement:\n{problem_statement}\n\nTest Cases:\n{test_code}\n\nAnalyze these test cases and extract all requirements for the solution."}
    ]
    ...
    try:
        ...
        logger.error(f"Test requirement analysis failed: {e}")
        return ""
...
def process_create_task(input_dict, enable_pev: bool = True, enable_mcts: bool = True):
    ...
    # NEW STEP: Generate and analyze test cases FIRST
    logger.info("=== STEP 1: Generating test cases ===")
    test_cases = generate_test_files(problem_statement, "", code_skeleton)
    test_files = extract_and_write_files(test_cases)
    logger.info(f"Created {len(test_files)} test files: {test_files}")
    
    # NEW STEP: Analyze test requirements
    logger.info("=== STEP 2: Analyzing test requirements ===")
    test_requirements = analyze_test_requirements_for_solution(test_cases, problem_statement)
    logger.info("Test requirements extracted for solution generation")
    
    # Generate initial solution with test context
    logger.info("=== STEP 3: Generating solution based on test requirements ===")
    initial_solution = generate_initial_solution_with_test_context(problem_statement, code_skeleton, test_cases, test_requirements)
    ...
    try:
        coverage_analysis = analyze_test_coverage(
            problem_statement, 
            test_cases,  # Use the test cases we generated in Step 1 
...
def fix_task_solve_workflow(problem_statement: str, *, timeout: int, run_id_1: str,\
    test_runner: str = "pytest", test_runner_mode: str = "FILE", n_max_steps = MAX_FIX_TASK_STEPS, enable_pev: bool = True, enable_mcts: bool = True) -> tuple[str, List[str], List[str]]:
    global run_id
    run_id=run_id_1
    ...
    tool_manager=FixTaskEnhancedToolManager(
        available_tools=[
            ...
            "generate_and_repair_test",
            "validate_fail_to_pass",
