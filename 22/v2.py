...
class Utils:
    ...
    def clean_code_response(response: str) -> str:
        """Clean code response by removing markdown code blocks and extra whitespace.
    ...
    def validate_python_filename(text: str) -> bool:
        ...
        Returns:
            True if first line looks like a Python filename
        """
        if not text:
            return False
...
class PromptManager():
    ...
    def create_system_user_messages(cls, system_prompt: str, user_content: str) -> list:
        """Create standard message format for LLM calls."""
        return [
    ...
    SYSTEM_PROMPT = textwrap.dedent("""
        <system_prompt>
            <role>
                ...
                You have access to tools to interact with the engineer's codebase.
            </role>

            <environment>
                <operating_system>Linux</operating_system>
                <working_directory>Repository root - DO NOT change directory</working_directory>
            </environment>

            <workflow>
                # Problem Solving Workflow

                Follow these steps in order to solve the problem statement:

                ## Step 1: Explore and Familiarize
                - Use `bash` tool to explore the project structure (`ls`, `find`, `tree`)
                - Read relevant files to understand the codebase
                - Identify files related to the problem statement
                - Understand the project architecture and dependencies
                - **CRITICAL**: Always use `sequential_thinking` tool BEFORE taking any action

                ## Step 2: Find Test Framework and Command
                - Identify the testing framework used (pytest, unittest, etc.)
                - Look for test configuration files (pytest.ini, setup.py, tox.ini, etc.)
                - Find the correct command to run existing tests
                - Verify the test command works by running it
                - Document the test command for later use

                ## Step 3: Generate Reproduction Test Script
                - **BEFORE fixing**: Generate a test script that reproduces the problem
                - Use `str_replace_editor` to create the test file
                - Run the test script to verify it FAILS (reproducing the issue)
                - Generate comprehensive test cases covering all aspects of the problem statement
                - Ensure if these tests pass, the problem is completely solved

                ## Step 4: Create TODO List
                - Use `sequential_thinking` to analyze the problem deeply
                - Create a detailed TODO list with specific tasks
                - Break down complex problems into manageable steps
                - Consider edge cases and potential side effects

                ## Step 5: Implement Solution
                - Follow the TODO list created in Step 4
                - Use `str_replace_editor` to update the code
                - Make minimal, focused changes to solve the problem
                - Ensure changes don't break existing functionality
                - Add appropriate error handling

                ## Step 6: Validate with Reproduction Test
                - Run the reproduction test script from Step 3
                - Verify the test now PASSES (problem is fixed)
                - If test fails, iterate from Step 4

                ## Step 7: Validate with Existing Tests (Pass-to-Pass Validation)
                
                **Objective**: Ensure your fix doesn't break existing functionality
                
                ### 7.1 Analyze Project Structure
                - Use `find` and `grep` to identify ALL test files in the project
                - Look for common test file patterns:
                  * Python: `test_*.py`, `*_test.py`, `tests/*.py`
                  * JavaScript: `*.test.js`, `*.spec.js`, `__tests__/*.js`
                  * Other patterns based on project conventions
                - Identify test directories: `tests/`, `test/`, `__tests__/`, `spec/`
                
                ### 7.2 Detect Testing Framework
                - Check configuration files to identify testing framework:
                  * Python: `pytest.ini`, `setup.py`, `pyproject.toml`, `tox.ini`
                  * JavaScript: `package.json`, `jest.config.js`, `mocha.opts`
                - Look for framework imports in test files
                - Common frameworks: pytest, unittest, Jest, Mocha, etc.
                - Document the framework and test command
                
                ### 7.3 Run All Tests (Quick Pass - No Detailed Logs)
                - Run all existing tests with minimal output to identify failures
                - Use framework-specific options for brief output:
                  * pytest: `pytest -q` or `pytest --tb=no`
                  * unittest: `python -m unittest discover -q`
                  * Jest: `npm test -- --silent`
                - **Goal**: Get a list of which test files/cases failed
                - **DO NOT** analyze detailed errors yet
                
                ### 7.4 Identify Failed Test Files
                - Parse test output to extract failed test file names
                - Create a list of failed test files only
                - If NO failures: Proceed to Step 8
                - If failures exist: Continue to 7.5
                
                ### 7.5 Rerun Failed Tests with Detailed Logs
                - For EACH failed test file, rerun with verbose output:
                  * pytest: `pytest -vv path/to/failed_test.py`
                  * unittest: `python -m unittest path.to.failed_test -v`
                  * Jest: `npm test -- path/to/failed_test.js --verbose`
                - Capture detailed error messages, stack traces, and assertions
                - Document each failure for analysis
                
                ### 7.6 Analyze Failures
                - Use `sequential_thinking` to analyze each failure
                - Determine if failure is:
                  * **Regression**: Your changes broke existing functionality → Go to Step 4
                  * **Pre-existing**: Test was already failing → Document and continue
                  * **Environment**: Missing dependencies/setup → Check Rule 1 (no installs)
                - Only iterate to Step 4 if your changes caused the regression
                
                ### 7.7 Validation Complete
                - Confirm: NO new regressions introduced by your changes
                - Proceed to Step 8 for comprehensive testing

                ## Step 8: Comprehensive Testing
                - Run comprehensive test suite (all tests)
                - Verify all F2P (Fail-to-Pass) tests pass
                - Verify all P2P (Pass-to-Pass) tests pass
                - Document test results

                ## Step 9: Error Handling
                - If ANY test fails, iterate from Step 4
                - Use `sequential_thinking` to analyze failures
                - Update TODO list with new findings
                - Implement fixes and revalidate

                ## Step 10: Completion
                - Once ALL tests pass (reproduction + existing + comprehensive)
                - Verify no errors are introduced
                - Call the `complete` tool to signal completion
            </workflow>

            <critical_rules>
                # Critical Rules - MUST FOLLOW

                ## Rule 1: No Package Installation
                - **NEVER** try to install packages (pip, apt, npm, etc.)
                - You do NOT have internet access
                - Work only with existing dependencies

                ## Rule 2: Do Not Edit Existing Tests
                - **NEVER** modify or delete existing test code
                - Only CREATE new test scripts for reproduction
                - Preserve all existing test functionality

                ## Rule 3: Find Test Command from Project
                - **MUST** discover how to run tests from project structure
                - Look for configuration files, README, setup files
                - Do NOT assume the test command

                ## Rule 4: Always Think Before Acting
                - **MUST** call `sequential_thinking` tool BEFORE any other tool
                - Think about: current step, purpose, context, next action
                - Plan before executing

                ## Rule 5: Static Paths Only
                - **NEVER** use `cd` to change directory
                - Always use static file/folder paths
                - All paths relative to repository root

                ## Rule 6: Track Your Progress
                - Always output which step you are in
                - State the purpose of the current step
                - Reference necessary context from previous actions
                - Use `sequential_thinking` to maintain context

                ## Rule 7: Anti-Repetition
                - **NEVER** repeat the same tool call more than 2 times
                - If a command fails twice, try a different approach
                - If file doesn't exist, CREATE it instead of retrying
                - Check file existence BEFORE running commands

                ## Rule 8: Validation 
                - **MUST** validate the test cases by running test code with bash tool and make sure bad formatted test code is not leng
            </critical_rules>
            ...
            <restrictions>
                ...
                - ❌ NO internet access
                - ❌ NO package installation (pip, apt, npm, etc.)
                - ❌ NO downloading files (wget, curl, etc.)
                - ❌ NO modifying existing test files
                - ❌ NO changing working directory (cd command)
                - ❌ NO repeating failed commands without changes
            </restrictions>

            <completion_criteria>
                # When to Call Complete Tool

                You are finished when:
                1. ✅ All reproduction tests PASS
                2. ✅ All existing tests PASS
                3. ✅ All comprehensive tests PASS
                4. ✅ No new errors introduced
                5. ✅ Problem statement fully addressed

                Then and only then, call the `complete` tool.
            </completion_criteria>
...
class Agent:
    ...
    def run_agent(
        ...
        system_prompt = PromptManager.SYSTEM_PROMPT.format(project_tree_structure=get_directory_tree())
...
def generate_solution_with_multi_step_reasoning(problem_statement: str, code_skeleton: str) -> str:
    ...
    while retry < 10:
        try:
            ...
            # Step 2: Infinite Loop Check and Validation
            loop_check_messages = PromptManager.create_system_user_messages(
            ...
            # Clean up the final response using utility method
...
def generate_initial_solution(problem_statement: str, code_skeleton: str) -> str:
    ...
    while retry < 10:
        try:
            ...
            if solution:
                ...
                # Clean up the response using utility method
    ...
    logger.error("Failed to generate initial solution after max retries")
    return ""
...
def generate_testcases_with_multi_step_reasoning(problem_statement: str, files_to_test: str, code_skeleton: str) -> str:
    retry = 0
    test_generation_messages = PromptManager.create_system_user_messages(
    ...
    while retry < 10:
        try:
            ...
            # Step 2: Testcase validation
            testcases_check_messages = PromptManager.create_system_user_messages(
            ...
            # Validate filename
            if not Utils.validate_python_filename(testcases):
                retry += 1
                ...
                logger.warning(f"Retrying because the first line is not a python test file name")
                continue

            logger.info("Multi-step reasoning testcase generation completed successfully with validation")
            return testcases
        except Exception as e:
            retry += 1
            logger.error(f"Exception in generate_testcases_with_multi_step_reasoning: {e}")
            time.sleep(2)
    
    logger.error("Multi-step reasoning testcase generation failed after max retries")
    return ""
    ...
    while retry < 10:
        try:
            ...
            if testcases:
                ...
                # Clean up the response using utility method
                testcases = Utils.clean_code_response(response)
                
                logger.info("Generated testcases successfully using fallback approach")
                return testcases
            
        except Exception as e:
            logger.error(f"Error generating testcases: {str(e)}")
            retry += 1
            time.sleep(2)
    
    logger.error("Failed to generate testcases after max retries")
    return ""
...
def check_problem_type(problem_statement: str) -> str:
    ...
    while retry < 10:
        try:
            ...
            logger.error(f"Error checking problem type: {e}")
            retry += 1
