...
**Context**: Now that you understand the structure, analyze the specific problem
**Rules**:
- Focus on the exact issue described in the problem statement
- Use test failures as precise indicators of what needs to be fixed
- Trace the problem to its root cause

**Required Actions**:
1. Use `search_in_specified_file_v2` to examine specific files in detail
2. Use `run_repo_tests` to understand current test failures
3. Use `run_code` to test specific code snippets and reproduce issues
4. Use `get_file_summary` to understand file purposes and relationships

**Tools to Use**: `search_in_specified_file_v2`, `run_repo_tests`, `run_code`, `get_file_summary`
**Success Criteria**: You understand the root cause and can identify the exact code that needs to be changed

---

## STEP 3: IMPACT ANALYSIS AND DEPENDENCY MAPPING
...
## STEP 4: SOLUTION DESIGN AND PLANNING
...
4. Never edit/update the existing test files directly when validating a hypothesis. Instead, when you need a new or focused test to reproduce or protect the fix, use the dedicated test generation tool.
...
**Context**: Verify that your changes work correctly and don't break anything
**Rules**:
- Ensure the updated code really fixes the problem and passes all tests.
- Ensure the updated code doesn't break anything eles and passes all tests before you use it to fix the problem. Here you need to find the test files corresponding to the updated code and validate the solution.
- If you get dependency errors, use `run_code` to validate the solution.
- Run tests to verify functionality
- Check for syntax errors of solution
...
4. Use `generate_test_function` to add test coverage
5. Never edit/update the existing test files directly when validating a hypothesis. Instead, when you need a new or focused test to reproduce or protect the fix, use the dedicated test generation tool.

**Tools to Use**: `run_code`, `run_repo_tests`
**Success Criteria**: All tests pass and the solution works correctly
...
## Exception Handling Requirements (Apply in Step 5):
...
## Multi-file Coordination Requirements (Apply in Steps 3-5):
...
## Testing and Validation Requirements (Apply in Steps 2, 6-7):
...
<code_quality_rules>
## Code Quality Requirements (Apply in Steps 4-5):
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    @EnhancedToolManager.tool
    def find_method_overrides(self, method_name: string) -> str:
        '''
