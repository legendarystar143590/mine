...
libraries = ["aifc", "argparse", "array", "ast", "asynchat", "asyncio", "asyncore", "bdb", "binascii", "bisect", "builtins", "bz2", "calendar", "cgi", "cgitb", "chunk", "cmath", "cmd", "code", "codecs", "codeop", "collections", "colorsys", "compileall", "concurrent", "configparser", "contextlib", "contextvars", "copy", "copyreg", "crypt", "csv", "ctypes", "curses", "dataclasses", "datetime", "dbm", "decimal", "difflib", "dis", "doctest", "email", "encodings", "ensurepip", "enum", "errno", "faulthandler", "fcntl", "filecmp", "fileinput", "fnmatch", "fractions", "ftplib", "functools", "gc", "getopt", "getpass", "gettext", "glob", "graphlib", "gzip", "hashlib", "heapq", "hmac", "html", "http", "imaplib", "imghdr", "importlib", "inspect", "io", "ipaddress", "itertools", "json", "keyword", "lib2to3", "linecache", "locale", "logging", "lzma", "mailbox", "mailcap", "marshal", "math", "mimetypes", "modulefinder", "msilib", "multiprocessing", "netrc", "nntplib", "numbers", "operator", "optparse", "os", "pathlib", "pdb", "pickle", "pickletools", "pipes", "pkgutil", "platform", "plistlib", "poplib", "posix", "pprint", "profile", "pstats", "pty", "pwd", "py_compile", "pyclbr", "pydoc", "queue", "quopri", "re", "readline", "reprlib", "resource", "rlcompleter", "runpy", "sched", "secrets", "select", "selectors", "shelve", "shlex", "shutil", "signal", "site", "smtpd", "smtplib", "sndhdr", "socket", "socketserver", "sqlite3", "ssl", "stat", "statistics", "string", "stringprep", "struct", "subprocess", "sunau", "symtable", "sys", "sysconfig", "tabnanny", "tarfile", "telnetlib", "tempfile", "termios", "textwrap", "threading", "time", "timeit", "tkinter", "token", "tokenize", "trace", "traceback", "tracemalloc", "tty", "turtle", "types", "typing", "unicodedata", "unittest", "urllib", "uu", "uuid", "venv", "warnings", "wave", "weakref", "webbrowser", "wsgiref", "xdrlib", "xml", "xmlrpc", "zipapp", "zipfile", "zipimport", "zlib", "zoneinfo"]
...
3. EDGE CASES & BOUNDARY CONDITIONS:
   ...
   - Detect off-by-one and boundary errors
...
"""
You are a premier Python developer. Your task is to produce an ABSOLUTE, FULLY FUNCTIONAL solution for the specified problem.

MISSION-CRITICAL: Each method MUST BE FULLY IMPLEMENTED - NO stubs, NO 'pass' statements, NO placeholders.

DETAILED INSTRUCTIONS:
1. Provide FULL file content and names in this format:
   ```python
   filename.py
   <complete code here>
...
2. **LIBRARY RESTRICTION**: Only allowed libraries:
   {libraries}
   - Confirm EVERY import is allowed

3. IMPLEMENTATION DETAILS:
   - Replace ALL 'pass' statements with real code
   - Full implementations of ALL functions
   - NO method left as only 'pass'
   - ALL functions MUST have working logic
   - Initiate all variables in __init__
   - Return real values, NEVER 'pass'

4. Maintain original names from the stub and fully implement them

5. Must handle EVERY edge case:
   - Empty inputs ([], "", None)
   - Single elements ([single])
   - Maximal/minimal values
   - Error condition handling (appropriate exceptions)

6. Solution must be executable and pass tests immediately

7. Multiline text issue:
   Use raw strings (not lists) unless explicitly requested

```python
main.py
class Node:
    def __init__(self, value):
        pass  # ❌ WRONG! Needs implementation!
    
    def value(self):
        pass  # ❌ WRONG! Must return self._value!
```

Return ONLY the Python code with file names. No explanations outside the code.
""")
...
"""
You are an expert code reviewer specializing in library constraint validation. Your task is to analyze the generated Python code and verify that ALL import statements only use libraries from the allowed list.
**CRITICAL LIBRARY CONSTRAINT (HIGHEST PRIORITY):**
You can ONLY import libraries from this EXACT list: {libraries}
**Your Tasks:**
1. Analyze ALL import statements in the code (both 'import module' and 'from module import ...' statements)
2. Verify each imported module is in the allowed list
3. If ANY import uses a disallowed library:
   - You MUST rewrite that part using ONLY allowed libraries
   - Implement missing functionality manually using only allowed libraries
   - Use alternative allowed modules where possible
   - If needed, implement algorithms from scratch using only standard allowed libraries
**If you find library violations:**
- Provide a corrected version of the code
- Remove ALL disallowed library imports
- Replace with implementations using only allowed libraries
- Ensure functionality is preserved
**If no library violations are detected:**
- Return the original code unchanged
**STRICT REQUIREMENT:** Return the final Python code along with file names. Do not include any explanations, comments, or additional text.
example:
```python
...
Think which step you are in and what you need to do next.
...
class EnhancedNetwork:
    ...
    @classmethod
    def inference(cls, messages: List[Dict[str, Any]], model: str, run_id: str = "1", temperature:float=0.0) -> dict:
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def run_code(self,content:str,file_path:str)->str:
        ...
        # Note: Third-party dependency checking is disabled to allow local modules
        # Original check was: if disallowed_modules: raise error
    ...
    def finish(self, investigation_summary: str):
        ...
        '''
        # Log the investigation summary for tracking purposes
        logger.info(f"Workflow completed with summary: {investigation_summary[:200]}...")
...
def agent_main(input_dict: Dict[str, Any], repo_dir: str = "repo", test_mode: bool = False):
    ...
    global DEFAULT_PROXY_URL, DEFAULT_TIMEOUT, MAX_TEST_PATCH_TIMEOUT, RUN_ID, run_id
    RUN_ID = os.getenv("RUN_ID", "")
    run_id = os.getenv("RUN_ID", "")
...
def check_problem_type(problem_statement: str) -> str:
    ...
    while retry < 10:
        try:
            ...
            if PROBLEM_TYPE_FIX in response:
...
def extract_files_from_solution(solution: str) -> dict:
    """Extract files from solution format - simplified version"""
    files = {}
    lines = solution.split('\n')
    current_filename = None
    current_content = []
...
def is_solution_valid(solution: str) -> bool:
    """Quick validation to check if solution looks reasonable"""
    
    if not solution or len(solution.strip()) < 10:
        logger.warning("[VALIDATION] Solution too short")
        return False
    
    lines = solution.split('\n')
    
    # Check format - should start with filename
    if not lines[0].strip().endswith('.py'):
        logger.warning("[VALIDATION] Solution doesn't start with .py filename")
        return False
    
    # Check for actual code content
    has_function_or_class = any('def ' in line or 'class ' in line for line in lines)
    if not has_function_or_class:
        logger.warning("[VALIDATION] Solution has no functions or classes")
        return False
    
    # Check for basic syntax (try to parse each file)
    try:
        files = extract_files_from_solution(solution)
        for filename, content in files.items():
            if content.strip():  # Only check non-empty files
                ast.parse(content, filename=filename)
        logger.debug("[VALIDATION] Syntax validation passed")
        return True
    except SyntaxError as e:
        logger.warning(f"[VALIDATION] Syntax error found: {e}")
        return False
    except Exception as e:
        logger.warning(f"[VALIDATION] Validation error: {e}")
        return False

def generate_single_model_solution(problem_statement: str, code_skeleton: str, model: str) -> str:
    """Generate solution using specific model with comprehensive multi-stage validation"""
    global libraries
    
    # Stage 1: Initial code generation
    logger.info("[VALIDATION 1/5] Generating initial solution...")
    code_generation_messages = [
        {
            "role": "system",
    ...
    code_response = EnhancedNetwork.make_request(code_generation_messages, model=model)

    # Stage 2: Library constraint check
    logger.info("[VALIDATION 2/5] Checking library constraints...")
    ...
    # Stage 3: Infinite loop detection
    logger.info("[VALIDATION 3/5] Detecting infinite loops...")
    loop_check_messages = [
        {
            "role": "system",
    ...
    # Stage 4: Protocol and pattern validation (iterators, callbacks, state management)
    logger.info("[VALIDATION 4/5] Validating protocols and patterns...")
    ...
    # Stage 5: Final correctness validation (algorithm logic, semantics, edge cases)
    logger.info("[VALIDATION 5/5] Final correctness validation...")
...
def process_create_task_with_best_model(problem_statement: str, code_skeleton: str, timeout: int, run_id_1: str) -> str:
    """Use the best model (Qwen3-Coder) with full iterative fixing workflow"""
    
    model = QWEN_MODEL_NAME
    initial_solution = generate_single_model_solution(problem_statement, code_skeleton, model)
    
    # Basic validation
    if not is_solution_valid(initial_solution):
        logger.warning(f"[BEST-MODEL] {model} failed basic validation, trying next model")
        return ""
    
    # Write solution files
    created_files = extract_and_write_files(initial_solution)
    logger.info(f"[BEST-MODEL] {model} created {len(created_files)} files")
    
    # Generate test cases
    test_cases = generate_test_files(problem_statement, created_files, code_skeleton)
    logger.info(f"[BEST-MODEL] {model} generated {len(test_cases)} test cases")
    logger.info(f"[BEST-MODEL] {model} test cases: {test_cases}")
    ...
    if patch == "limit":
        logger.debug(f"🚀[BEST-MODEL] Patch is None, cleaning up failed attempt and generating initial solution")
        cleanup_failed_attempt(created_files)
...
def cleanup_failed_attempt(created_files: list):
    """Clean up files from failed model attempt"""
    try:
        # Reset git to clean state
        os.system("git reset --hard")
        
        # Remove any created files that might not be tracked
        ...
        logger.info("[CLEANUP] Cleaned up failed attempt files")
    except Exception as e:
...
def process_create_task(input_dict):
    ...
    # Calculate timeout for the entire process
    total_timeout = DEFAULT_TIMEOUT - (time.time()-start_time) - 60
    
    # Use best model with full iterative fixing
    logger.info("[CREATE] Starting best model workflow with full iterative fixing")
    
    patch = process_create_task_with_best_model(
        problem_statement, 
        code_skeleton, 
        total_timeout,
        run_id,
