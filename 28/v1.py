...
PHASE_SPECIFIC_GUIDANCE = {
    ...
    PHASE_IMPLEMENTATION: textwrap.dedent(
        """
            ...
            - Make precise, targeted code changes using str_replace_editor
...
FIX_TASK_SYSTEM_PROMPT = textwrap.dedent(
    ...
    <validation_requirements>
        ...
        **Phase A: Fail-to-Pass (F2P) Validation (USE ISOLATED TESTS)**
        - Run YOUR isolated test script (the one you created with mocking)
        - This test uses pytest + mocking to work independently
        - Confirm it NOW PASSES (it should have failed before the fix)
        - Execute: `bash("python -m pytest test_your_file.py -v")`
        - If it still fails → iterate on the fix
        - This validates the specific bug is fixed
        
        **Phase B: Pass-to-Pass (P2P) Validation (TRY BUILT-IN FRAMEWORK FIRST)**
        - **Step 1: Try Native Test Framework**
          - Find ALL relevant existing test files
          - **CRITICAL: Discover and use the project's native test framework**
          - Execute: `bash("python -m pytest tests/ -v")` OR `bash("python -m unittest discover tests")`
          - Run existing test suite using native framework
          
        - **Step 2: If Dependency Errors Occur:**
          - Document the exact error (missing packages, import errors, etc.)
          - Only if dependencies cannot be installed due to no internet access
          - Then fall back to isolated test approach with mocking
          - Create versions of existing tests with necessary mocks
          - Ensure ALL existing test scenarios are covered
          
        - **Step 3: Validation Requirements:**
          - ALL tests MUST pass - no exceptions
          - Both your new test AND existing tests must pass
          - If ANY test fails:
            * Analyze why your fix caused the failure
            * Use debugging strategies to trace the issue
    ...
    <isolated_test_creation>
        **CRITICAL: Creating Standalone Test Scripts that Work in Isolation**
        
        **⚠️ IMPORTANT: When to Use Isolated Tests**
        
        **For Fail-to-Pass Validation (Step 8):**
        - ✅ **ALWAYS** use isolated test creation with pytest + mocking
        - Create your reproduction test with minimal dependencies
        - Mock external dependencies that may not be available
        - Focus on reproducing the specific bug
        
        **For Pass-to-Pass Validation (Step 9):**
        - ✅ **FIRST**: Try built-in test framework (pytest, unittest, etc.)
        - ✅ **ONLY** use isolated approach if dependency errors occur
        - Run: `bash("python -m pytest tests/ -v")` to run existing tests
        - If this fails due to missing dependencies that cannot be installed:
          - Document the dependency error
          - Then use isolated test approach to mock problematic dependencies
        - Run existing test files with mocks to ensure no regressions
        
        **Why Test in Isolation?**
        - Some projects have complex dependencies that cannot be installed
        - Environment may lack certain packages or services
        - Tests need to run independently without external dependencies
        - Reproduce bugs without full project setup
        
        **Strategy: Pytest + Mocking to Create Isolated Tests**
        
        **STEP 1: Analyze Dependencies**
        - Identify which modules/components the failing code depends on
        - Read existing tests to understand what needs to be mocked
        - Use `search_in_all_files_content` to find import statements
        - List all external dependencies that may not be available
        
        **STEP 2: Design Test Structure**
        ```python
        # Example structure for isolated test
        import pytest
        from unittest.mock import Mock, patch, MagicMock
        import sys
        
        # Step 2.1: Mock external dependencies first
        @pytest.fixture(autouse=True)
        def mock_dependencies():
            '''Mock unavailable or problematic dependencies'''
            with patch('module.external_dependency') as mock_ext:
                mock_ext.return_value = 'mocked_value'
                yield mock_ext
        
        # Step 2.2: Create fixtures for test data
        @pytest.fixture
        def sample_data():
            return {'key': 'value'}
        
        # Step 2.3: Parametrize for edge cases
        @pytest.mark.parametrize("input,expected", [
            (normal_case, expected_normal),
            (edge_case_1, expected_edge_1),
            (edge_case_2, expected_edge_2),
        ])
        ...
        **STEP 3: Mock Implementation Guide**
        ...
        3.3 **Mock Database Operations**
        ```python
        @patch('module.db.execute')
        def test_db_query(mock_db):
            mock_db.return_value = [{'id': 1, 'name': 'test'}]
            # Your test code
        ```
        
        3.4 **Mock Environment Variables**
        ```python
        ...
        **STEP 4: Create Minimal Reproducing Test**
        ...
        class TestBugReproduction:
            def test_specific_failure_case(self):
                # Minimal setup to reproduce the exact bug
                input_data = {...}
        ...
        4.2 **Critical Rules:**
        - Mock ALL unavailable dependencies BEFORE importing
        - Use pytest fixtures for common test setup
        - Parametrize to cover multiple scenarios
        - Keep tests focused on ONE specific issue
        - Make tests deterministic (no random data)
        
        **STEP 5: Verify Test Executes Standalone**
        
        5.1 **Test that it runs without dependencies:**
        ```bash
        python -m pytest test_bug_reproduction.py -v
        ```
        
        5.2 **Verify the test FAILS before fix:**
        - The test must reproduce the exact error
        - Error message should match the problem statement
        - Stack trace should point to the bug location
        
        5.3 **After fix, verify test PASSES:**
        - Same test should now pass
        - No other tests should fail
        - Run multiple times to ensure reliability
        
        **STEP 6: Common Patterns**
        
        6.1 **Django/ORM Testing:**
        ```python
        from django.test import TestCase
        from unittest.mock import patch
        
        @patch('module.Model.objects.get')
        def test_orm_operation(mock_get):
            mock_get.return_value = MockModel(id=1)
            # Your test
        ```
        
        6.2 **Mocking Class Instances:**
        ```python
        ...
        6.3 **Context Manager Mocking:**
        ```python
        mock_context = Mock()
        mock_context.__enter__.return_value = mock_data
        mock_context.__exit__.return_value = None
        with patch('builtins.open', return_value=mock_context):
            # Your test
        ```
        
        **STEP 7: Debugging Failed Tests**
        
        - Use `pytest -v` for verbose output
        - Add `print()` statements to see intermediate values
        - Use `pytest --pdb` to drop into debugger
        - Check that mocks are being called correctly
        - Verify test data matches actual usage
        
        **STEP 8: Integration with Existing Test Framework**
        
        - Place test file in appropriate test directory
        - Follow existing naming conventions
        - Use same test base class if present
        - Ensure test is discoverable by pytest
        - Run full test suite to check compatibility
    </isolated_test_creation>
    ...
    <detailed_workflow_steps>
        ...
        **1.1 Read Problem Statement**
        - Extract key information: what fails, expected vs actual behavior
        - Identify affected components, functions, or modules
        - Note any error messages or stack traces mentioned
        - Use `sequential_thinking` to document understanding
        
        **1.2 Explore Project Structure**
        - Use `list_directory` to understand project layout
        - Identify source code locations
        - Locate test directories
        - Find configuration files
        
        **1.3 Search for Related Code**
        - Use `search_in_all_files_content` to find all references to failing component
        - Identify where the buggy code is used
        - Map dependencies between modules
        
        **1.4 Read Relevant Files**
        - Use `get_file_content` to read source files
        - Use `get_context_around_line` to understand code flow
        - Trace through execution paths
        - Document findings with `save_context`
        
        **Step 2: Analyze Test Framework & Establish Baseline**
        
        **2.1 Identify Test Framework**
        - Read existing test files to detect framework (pytest, unittest, nose2, etc.)
        - Check for test configuration files (pytest.ini, setup.cfg, tox.ini)
        - Look for imports like `pytest`, `unittest`, `TestCase`
        
        **2.2 Run Existing Test Suite**
        - Execute `bash("python -m pytest tests/ -v")` or equivalent
        - Document which tests pass
        - Note test execution time and any warnings
        - Save baseline results with `save_context`
        
        **2.3 Understand Test Patterns**
        - Analyze fixture usage
        - Check parametrization patterns
        - Identify mocking strategies
        - Note assertion styles
        ...
        **3.1 Choose Location**
        ...
        - Ensure it's discoverable by test runner
        
        **3.2 Set Up Test Structure**
        - Import necessary modules (with mocking if needed)
        - Define fixtures for test data
        - Create test class or functions
        - Add parametrization for edge cases
        
        **3.3 Implement Test Logic**
        - Write minimal code to reproduce bug
        - Mock unavailable dependencies
        - Set up test data
        ...
        **4.2 Review Test Quality**
        - Test is focused on specific issue
        - Test is deterministic (no randomness)
        - Test uses appropriate assertions
        - Test follows existing patterns
        
        **4.3 Validate Mocking**
        - All external dependencies are mocked
        - Mocks return appropriate values
        - Mocks are configured correctly
        
        **Step 5: Run Test and Confirm It Fails**
        
        **5.1 Execute Test**
        - Run: `bash("python -m pytest test_file.py -v")`
        - Capture full output
        - Verify test runs (no import errors)
        
        **5.2 Confirm Failure**
        - Test MUST fail with expected error
        - Error matches problem description
        - Stack trace points to bug location
        - Save failure output with `save_context`
        
        **PHASE 3: FIX (Steps 6-7)**
        
        **Step 6: Analyze Root Cause & Plan Fix**
        
        **6.1 Trace Through Code**
        - Use `get_context_around_line` at error location
        - Understand why code fails
        - Identify incorrect logic, missing checks, or wrong assumptions
        
        **6.2 Root Cause Analysis**
        - Use `sequential_thinking` to document analysis
        - Consider edge cases
        - Think about type mismatches, null cases, boundary conditions
        - Plan minimal changes to fix issue
        
        **6.3 Design Solution**
        - Propose at least 2 different approaches
        - Use `get_approval_for_solution` to get approval
        ...
        **7.1 Make Code Changes**
        - Use `str_replace_editor` to edit files
        - Make MINIMAL changes
        - Preserve existing functionality
        - Follow code style of project
        
        **7.2 Verify Syntax**
        - Check file is still valid Python
        - Ensure imports are correct
        - Verify no syntax errors
        
        **7.3 Document Changes**
        - Save context with `save_context`
        - Note what was changed and why
        - Record before/after behavior
        
        **PHASE 4: VALIDATE (Steps 8-11)**
        
        **Step 8: Fail-to-Pass Validation (ALWAYS Use Isolated Tests)**
        
        **8.1 Run Your Reproduction Test**
        - Execute your isolated test script: `bash("python -m pytest test_file.py -v")`
        - Test MUST now PASS (it failed before the fix)
        - Capture output to verify
        - This validates the bug is fixed
        
        **8.2 Iterate if Needed**
        - If test still fails, analyze why
        - Use debugging strategies to trace the issue
        - Adjust fix and re-test
        - Don't move forward until YOUR test passes
        
        **Step 9: Pass-to-Pass Validation (Use Built-In Framework FIRST)**
        
        **9.1 Try Built-In Test Framework**
        - Execute all existing tests: `bash("python -m pytest tests/ -v")`
        - OR: `bash("python -m unittest discover tests")`
        - ALL existing tests must STILL pass
        - No regressions allowed
        
        **9.2 If Dependency Errors Occur:**
        - Document the exact dependency error
        - Check if package can be installed (if not, proceed to 9.3)
        - Read existing test files to understand what they test
        - Identify which dependencies cause issues
        
        **9.3 Use Isolated Approach as Fallback:**
        - Only if built-in framework fails due to uninstallable dependencies
        - Create version of existing tests with mocks where needed
        - Ensure all existing test scenarios are covered
        - Run tests with mocks to verify no regressions
        - Document why isolated approach was necessary
        
        **9.4 Handle Failures**
        - If any test fails, analyze why
        - Determine if fix broke something
        - Use debugging strategies to trace failures
        - Refine fix to satisfy both F2P and P2P
        ...
        **11.2 Call Finish**
        - Use `finish` tool with summary
        - Document what was fixed
        - Note any considerations for future
    ...
    <debugging_strategies>
        ...
        **When tests fail to pass after your fix:**
        
        1. **Read the Exact Error Output**
           - Copy the FULL error message from test output
           - Identify line numbers and stack traces
           - Note any ImportError, NameError, AttributeError, or assertion failures
           
        2. **Trace the Code Path**
           - Use `search_in_all_files_content` to find all locations where failing code is used
           - Use `get_context_around_line` to see surrounding code at error lines
           - Use `list_directory` to understand file organization
           - Read the actual source files to understand the flow
           
        3. **Analyze Root Cause**
           - Use `sequential_thinking` to document your analysis
           - Compare expected vs actual behavior
           - Identify what changed between before/after
           - Consider edge cases and boundary conditions
           
        4. **Iterative Fixing**
           - Make MINIMAL changes first
           - Test after each change
           - If a fix fails, revert and try a different approach
           - Document each attempt with `save_context`
           
        5. **Validation Depth**
           - Run failing test multiple times to ensure reproducibility
           - Check related test files for similar failures
           - Verify no side effects in other modules
           - Ensure compatibility with imports and dependencies
           
        **Common Failure Patterns:**
        - **Import Errors**: Check file paths, __init__.py presence, PYTHONPATH
        - **Attribute Errors**: Verify object types, method existence, inheritance chains
        - **Assertion Failures**: Understand expected vs actual values, check data types
        - **Type Errors**: Verify parameter types and return types match signatures
        - **Logic Errors**: Trace through execution flow, add debug prints via str_replace_editor
        
        **Tool Usage for Debugging:**
        - `search_in_all_files_content("class ClassName")` - Find all usages of a class
        - `search_in_all_files_content("def function_name")` - Find all definitions of a function
        - `get_file_content("path/to/file.py")` - Read complete file
        - `get_context_around_line("path/to/file.py", line_number, 10)` - See surrounding code
        - `bash("python test_file.py")` - Run tests to see actual errors
        - `list_directory("tests/")` - Explore test structure
    </debugging_strategies>
...
class Utils:
    ...
    @classmethod
    def parse_test_results_simple(cls, output: str) -> dict:
        """Parse test output for common test frameworks"""
        import re
        ...
        # Try to detect framework and parse
        # Pytest pattern
        pytest_summary = re.search(r'=+\s*(\d+) (passed|failed|error)', output)
        if pytest_summary or 'pytest' in output.lower():
            ...
            result['passed'] = int(total_match.group(1)) if total_match else 0
            ...
            failed_section = re.search(r'FAILED\s+(.+?)=+\s*\n', output, re.DOTALL)
            if failed_section:
                # Try to extract test names
                test_matches = re.findall(r'FAILED\s+(.+?)\n', failed_section.group(1))
                for test_name in test_matches:
                    result['failed_tests'].append({'name': test_name.strip(), 'error': ''})
        
        # Unittest pattern
        elif 'unittest' in output.lower() or 'Ran' in output and 'test' in output:
            result['framework'] = 'unittest'
            ran_match = re.search(r'Ran\s+(\d+)', output)
            result['total_tests'] = int(ran_match.group(1)) if ran_match else 0
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def __init__(self, available_tools: Optional[list[str]] = [], test_runner: str = "pytest",
        ...
        self.saved_context = []  # For tracking saved context entries
        self.temp_files = set()  # Track temporary files for git patch exclusion
    ...
    def add_temp_file(self, file_path: str):
        """Add a temporary file to the exclusion list for git patches"""
    ...
    def get_final_git_patch(self) -> str:
        ...
        try:
            ...
            try:
                ...
                pass
            # Add temp files to exclude
            try:
                for _p in getattr(self, "temp_files", set()):
                    exclude.add(os.path.relpath(_p))
            except Exception:
                pass
            ls = subprocess.run(
    ...
    def bash(self, command: str, is_test_command: bool = False) -> str:
        ...
        Arguments:
            ...
            is_test_command: Set to true when running test commands to get parsed output showing only failed tests. Set to false (default) for full output or non-test commands.
        '''
        import re
        
        # Banned commands check
        banned_command_strs = ["git init", "git commit", "git add"]
        for banned_str in banned_command_strs:
            if banned_str in command:
                raise EnhancedToolManager.Error(
                    EnhancedToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                    f"Command not executed due to banned string in command: {banned_str}"
                )
        ...
        try:
            ...
            # If it's a test command, parse the results
            if is_test_command:
                parsed_results = Utils.parse_test_results_simple(clean_output)
                
                if parsed_results:
                    formatted_output = f"""Test Command: {command}

    Test Summary:
    Total Tests: {parsed_results.get('total_tests', 0)}
    ...
    """
                    ...
                    if parsed_results.get('failed', 0) > 0 and parsed_results.get('failed_tests'):
                        formatted_output += "\nFailed Tests:\n" + "=" * 80 + "\n"
    ...
    @EnhancedToolManager.tool
    def save_context(self, step_number: int, step_name: str, summary: str, 
                     findings: str = "", next_steps: str = "") -> str:
        '''
        Save a summary or context for the current step to maintain continuity across the workflow.
        Arguments:
            step_number: The current step number (1-11)
            step_name: Name of the current step (e.g., 'Understand Problem', 'Create Test Script')
            summary: Summary of what was accomplished in this step
            findings: Key findings, discoveries, or important information from this step
            next_steps: What needs to be done in the next step(s)
        '''
        context_entry = {
        ...
        self.saved_context.append(context_entry)
        
        return f"Context saved for Step {step_number}: {step_name}"
    
    @EnhancedToolManager.tool
    def sequential_thinking(self, thought: str, thoughtNumber: int, totalThoughts: int, 
                            nextThoughtNeeded: bool, isRevision: bool = False, 
                            revisesThought: int = None, branchFromThought: int = None, 
                            branchId: str = None, needsMoreThoughts: bool = False) -> str:
        '''
        ...
        '''
        if not hasattr(self, 'thought_history'):
...
{'─' * 80}
    ...
    @EnhancedToolManager.tool
    def str_replace_editor(self, command: str, path: str, old_str: str = None, 
                         new_str: str = None, file_text: str = None, 
                         insert_line: int = None, view_range: list = None) -> str:
        '''
        ...
        Arguments:
            ...
            view_range: Optional parameter of 'view' command when 'path' points to a file. If none is given, the full file is shown. If provided, the file will be shown in the indicated line number range, e.g. [11, 12] will show lines 11 and 12
        '''
        ...
        try:
            if command == "view":
                ...
                # View file
                content = path_obj.read_text() if path_obj.exists() else ""
                ...
                if path_obj.exists() and path_obj.stat().st_size > 0:
                    raise EnhancedToolManager.Error(
                        EnhancedToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                        f"File already exists and is not empty at: {path}"
                    )
                
                path_obj.parent.mkdir(parents=True, exist_ok=True)
                path_obj.write_text(file_text)
                
                # Track created file for git patch exclusion if needed
                self.add_temp_file(str(path_obj))
                ...
                if occurrences > 1:
                    raise EnhancedToolManager.Error(
                        EnhancedToolManager.Error.ErrorType.MULTIPLE_SEARCH_RESULTS_FOUND.name,
                        f"Multiple occurrences of old_str found in {path}. Please ensure it is unique"
                    )
                ...
                if insert_line < 0 or insert_line > n_lines:
                    raise EnhancedToolManager.Error(
                        EnhancedToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                        f"Invalid insert_line parameter: {insert_line}. It should be within [0, {n_lines}]"
                    )
                ...
                return f"File {path} has been edited. Inserted content at line {insert_line}"
            
            elif command == "undo_edit":
                if not hasattr(self, '_file_history'):
                    self._file_history = {}
                
                if path not in self._file_history or not self._file_history[path]:
                    raise EnhancedToolManager.Error(
                        EnhancedToolManager.Error.ErrorType.UNKNOWN.name,
                        f"No edit history found for {path}"
                    )
                
                old_content = self._file_history[path].pop()
                path_obj.write_text(old_content)
...
class CreateTaskEnhancedToolManager(FixTaskEnhancedToolManager):
    """Enhanced Tool Manager for CREATE tasks with advanced tools"""
    
    def __init__(self, available_tools: Optional[list[str]] = None, test_runner: str = "pytest",
                 test_runner_mode: str = "FILE"):
        # Initialize with default tools for create tasks
        ...
        super().__init__(available_tools=available_tools, test_runner=test_runner, 
                        test_runner_mode=test_runner_mode)
        
        # Create tasks don't need approval for solutions
        self.is_solution_approved = True
...
def process_create_task(input_dict, enable_pev: bool = True, enable_mcts: bool = True):
    ...
    # Use create_task_solve_workflow for CREATE tasks
    patch = create_task_solve_workflow(
        problem_statement,
        timeout=timeout,
        run_id_1=run_id,
...
def fix_task_solve_workflow(
    ...
    *, 
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 Starting FIX TASK workflow")
    logger.info(f"📋 Strategy: {strategy.get('name', 'Default')}")
    logger.info(f"🎯 MCTS Path: {' -> '.join(mcts_path[:5]) if mcts_path else 'None'}")
    logger.info(f"{'='*80}\n")
    ...
    for step in range(n_max_steps):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"STEP {step + 1}/{n_max_steps} | Phase: {phase_manager.current_phase}")
        logger.info(f"Time: {elapsed:.2f}s / {timeout}s")
        logger.info(f"{'='*80}\n")

        if use_multi_phase and step > 0:
            should_transition, new_phase = phase_manager.should_transition(step, cot)
            if should_transition:
                logger.info(f"🔄 Phase transition: {phase_manager.current_phase} -> {new_phase}")
