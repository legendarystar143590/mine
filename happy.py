...
{
  "status": "perfect",
  "message": "All test cases are valid and comprehensive"
}
...
If tests need UPDATES (issues found or missing cases):
{
  ...
  "test_code": "FULL UPDATED TEST CODE WITH FILE NAMES"
}

CRITICAL RULES:
- Output ONLY valid JSON, nothing else
- No markdown code blocks, no explanations outside JSON
- If status is "updated", test_code field MUST contain the complete test code in this format:
  test_a.py
  contents of test_a.py
  
  test_b.py
  contents of test_b.py
- Do not wrap test_code content in markdown blocks
- Ensure JSON is properly escaped (newlines as \\n in JSON strings)
"""
)
...
def generate_testcases_with_multi_step_reasoning(problem_statement: str, files_to_test: str, code_skeleton: str) -> str:
    ...
    while retry < 10:
        try:
            testcode_response = EnhancedNetwork.make_request(test_generation_messages, model=QWEN_MODEL_NAME, temperature=0.7)
            ...
            testcode_checked_response = EnhancedNetwork.make_request(testcases_check_messages, model=DEEPSEEK_MODEL_NAME, temperature = 0.3)
            ...
            if status == "perfect":
                ...
                for validation_iteration in range(max_validation_iterations):
                    ...
                    try:
                        ...
                        revalidation_response = EnhancedNetwork.make_request(testcases_recheck_messages, model=DEEPSEEK_MODEL_NAME, temperature=0.3)
